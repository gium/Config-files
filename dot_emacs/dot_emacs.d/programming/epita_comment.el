;;
;; std_comment.el for Emacs in epita
;;
;; Made by Frederic Denis
;; Login   <fred@epita.fr>
;;
;; Started on  Thu Sep  9 23:34:05 1993 Frederic Denis
;; Last update Thu Jan 18 22:18:15 2007 Guillaume SADEGH
;;
;; Based on Comment routines by Isaac
;; Updated by Eole <eole@epita.fr> (2002).
;;
;;  o   correction by eole <Friday March  1 2002,  5:36:11>
;;  o   Correct the kill-ring annoyance (thank's to Didier Verna)
;;  o   Epita compliant
;;  o   emacs-21.x compliant
;;  o   Not eating kill-ring anymore
;;
;; INSTALLATION:
;; Call this file in your dot emacs file like this :
;;; (load-file "~/PATH/std_comment.el")
;;
...
(setq user-epita-login-name	"sadegh_g"
      user-epita-full-name	"Guillaume SADEGH")
...
(if (setq user-nickname user-epita-login-name)
    (setq user-nickname user-epita-full-name))
...
(setq std-c-alist               '( (cs . "/*") (cc . "** ") (ce . "*/") )
      ...
      std-ruby-alist            '( (cs . "#!/usr/bin/ruby")  (cc . "## ")(ce . "##") )
      std-python-alist          '( (cs . "#!/usr/bin/python")  (cc . "## ")(ce . "##") )
...
(defun std-file-header ()
  ...
  (let ((projname "toto")(location "tiuti"))
    ...
    (setq length-header (length (concat (std-get 'cc)
					(buffer-name)
					header-for
					projname
					header-in)))
    (setq length-location (length location))

    (if (>= (+ length-location length-header) 80)
	(setq location (substring location (- 78 length-header))))
