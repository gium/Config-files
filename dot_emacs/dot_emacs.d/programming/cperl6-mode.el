;;; cperl6-mode.el --- Perl code editing commands for Emacs
...
;; Maintainer: Ilya Zakharevich <cperl6@ilyaz.org>
...
;;; Subject: cperl6-mode: Another perl mode for Gnuemacs
...
;;; Corrections made by Ilya Zakharevich cperl6@ilyaz.org
;;; XEmacs changes by Peter Arius arius@informatik.uni-erlangen.de

;;; Commentary:

;; $Id: cperl6-mode.el 266 2006-09-06 09:36:40Z ss5 $

;;; If your Emacs does not default to `cperl6-mode' on Perl files:
;;; To use this mode put the following into
;;; your .emacs file:

;; (autoload 'perl-mode "cperl6-mode" "alternate mode for editing Perl programs" t)
...
;; or as help on variables `cperl6-tips', `cperl6-problems',         <<<<<<
;; `cperl6-non-problems', `cperl6-praise', `cperl6-speed'.            <<<<<<
...
;;;  indent-for-comment knows about bad cases, cperl6-indent-for-comment
;;;  moves cursor to a correct place.
;;;  cperl6-indent-exp written from the scratch! Slow... (quadratic!) :-(
...
;;;  Names of functions and variables changed to conform to `cperl6-' style.

;;;; After 1.5:
;;;  Some bugs with indentation of labels (and embedded subs) corrected.
;;;  `cperl6-indent-region' done (slow :-()).
;;;  `cperl6-fill-paragraph' done.
;;;  Better package support for `imenu'.
;;;  Progress indicator for indentation (with `imenu' loaded).
;;;  `Cperl6-set' was busted, now setting the individual hairy option
;;;     should be better.

;;;; After 1.6:
;;; `cperl6-set-style' done.
;;; `cperl6-check-syntax' done.
;;; Menu done.
;;; New config variables `cperl6-close-paren-offset' and `cperl6-comment-column'.
;;; Bugs with `cperl6-auto-newline' corrected.
;;; `cperl6-electric-lbrace' can work with `cperl6-auto-newline' in situation
;;; like $hash{.
...
;;;   `define-key's
;;; - replaced `mark-active' in menu definition by `cperl6-use-region-p'.
;;; Todo (at least):
;;; - use emacs-vers.el (http://www.cs.utah.edu/~eeide/emacs/emacs-vers.el.gz)
;;;   for portable code?
;;; - should `cperl6-mode' do a
;;;	(if (featurep 'easymenu) (easy-menu-add cperl6-menu))
;;;   or should this be left to the user's `cperl6-mode-hook'?
...
;;; `cperl6-font-lock-enhanced' deprecated. Use font-lock-extra.el.
...
;;;; After 1.12:
;;; `cperl6-lineup-step' and `cperl6-lineup' added: lineup constructions
;;; vertically.
;;; `cperl6-do-auto-fill' updated for 19.29 style.
;;; `cperl6-info-on-command' now has a default.
;;; Workaround for broken C-h on XEmacs.
;;; VC strings escaped.
;;; C-h f now may prompt for function name instead of going on,
;;; controlled by `cperl6-info-on-command-no-prompt'.
...
;;; `cperl6-close-paren-offset' affects ?\] too (and ?\} if not block)
...
;;;========================================  ==========  ============  ============
;;;cperl6-block-p                             469         3.7799999999  0.0080597014
;;;cperl6-get-state                           505         163.39000000  0.3235445544
;;;cperl6-comment-indent                      12          0.0299999999  0.0024999999
;;;cperl6-backward-to-noncomment              939         4.4599999999  0.0047497337
;;;cperl6-calculate-indent                    505         172.22000000  0.3410297029
;;;cperl6-indent-line                         505         172.88000000  0.3423366336
;;;cperl6-use-region-p                        40          0.0299999999  0.0007499999
;;;cperl6-indent-exp                          1           177.97000000  177.97000000
;;;cperl6-to-comment-or-eol                   1453        3.9800000000  0.0027391603
;;;cperl6-backward-to-start-of-continued-exp  9           0.0300000000  0.0033333333
;;;cperl6-indent-region                       1           177.94000000  177.94000000

;;;; After 1.15:
;;; Takes into account white space after opening parentheses during indent.
;;; May highlight pods and here-documents: see `cperl6-pod-here-scan',
;;; `cperl6-pod-here-fontify', `cperl6-pod-face'. Does not use this info
...
;;;  `cperl6-pod-head-face' introduced. Used for highlighting.
...
;;;  `cperl6-calculate-indent' dis suppose that `parse-start' was defined.
...
;;;  Anonymous subs are split by `cperl6-linefeed' as well.
;;;  Electric parens embrace a region if present.
;;;  To make `cperl6-auto-newline' useful,
;;;    `cperl6-auto-newline-after-colon' is introduced.
;;;  `cperl6-electric-parens' is now t or nul. The old meaning is moved to
;;;  `cperl6-electric-parens-string'.
;;;  `cperl6-toggle-auto-newline' introduced, put on C-c C-a.
;;;  `cperl6-toggle-abbrev' introduced, put on C-c C-k.
;;;  `cperl6-toggle-electric' introduced, put on C-c C-e.
;;;  Beginning-of-defun-regexp was not anchored.

;;;; After 1.21
;;;  Auto-newline grants `cperl6-extra-newline-before-brace' if "{" is typed
;;;    after ")".
...
;;;  `cperl6-find-bad-style' added.

;;;; After 1.25
;;;  When search for here-documents, we ignore commented << in simplest cases.
;;;  `cperl6-get-help' added, available on C-h v and from menu.
;;;  Auto-help added. Default with `cperl6-hairy', switchable on/off
;;;   with startup variable `cperl6-lazy-help-time' and from
...
;;;  Minor updates to `cperl6-short-docs'.
...
;;;  Shrinkwrapping of info, regulated by `cperl6-max-help-size',
;;;  `cperl6-shrink-wrap-info-frame'.
...
;;;  `cperl6-use-syntax-table-text-property'. Currently recognized:
...
;;;    'abc$'
;;;    sub a ($); sub a ($) {}
;;;  (provide 'cperl6-mode) was missing!
;;;  `cperl6-after-expr-p' is now much smarter after `}'.
;;;  `cperl6-praise' added to mini-docs.
;;;  Utilities try to support subs-with-prototypes.

;;;; After 1.32.1
;;;  `cperl6-after-expr-p' is now much smarter after "() {}" and "word {}":
...
;;;  Thanks for Greg Badros: `cperl6-lazy-unstall' is more complete,
;;;  `cperl6-get-help' is made compatible with `query-replace'.

;;;; As of Apr 15, development version of 19.34 supports
;;;; `syntax-table' text properties. Try setting
;;;; `cperl6-use-syntax-table-text-property'.
...
;;;  `cperl6-lineup' is put on C-M-| (=C-M-S-\\).
;;;  Start of `cperl6-beautify-regexp'.

;;;; After 1.32.4
;;; `cperl6-tags-hier-init' did not work in text-mode.
;;; `cperl6-noscan-files-regexp' had a misprint.
...
;;; `cperl6-indent-left-aligned-comments' created.
;;; Logic of `cperl6-indent-exp' changed a little bit, should be more
...
;;;  (governed by  `cperl6-use-syntax-table-text-property-for-tags').

;;;; After 1.35:
;;; Can process several =pod/=cut sections one after another.
;;; Knows of `extproc' when under `emx', indents with `__END__' and `__DATA__'.
;;; `cperl6-under-as-char' implemented (XEmacs people like broken behaviour).
;;; Beautifier for regexps fixed.
;;; `cperl6-beautify-level', `cperl6-contract-level' coded
;;;
...
;;;  `cperl6-find-pods-heres' somehow maybe called when string-face is undefined
...
;;;  `cperl6-add-tags-recurse', `cperl6-add-tags-recurse-noxs'
...
;;;  Debugging code in `cperl6-electric-keywords' was leaking a message;
...
;;;; After 2.3:
;;;  Added `cperl6-perldoc' (thanks to Anthony Foiani <afoiani@uswest.com>)
;;;  Added `cperl6-pod-to-manpage' (thanks to Nick Roberts <Nick.Roberts@src.bae.co.uk>)
;;;  All the function warnings go away.

;;;; After 2.4:
;;;  `Perl doc', `Regexp' submenus created (latter to allow short displays).
;;;  `cperl6-clobber-lisp-bindings' added.
;;;  $a->y() is not y///.
;;;  `cperl6-after-block-p' was missing a `save-excursion' => wrong results.
;;;  `cperl6-val' was defined too late.
;;;  `cperl6-init-faces' was failing.
;;;  Init faces when loading `ps-print'.

;;;; After 2.4:
;;;  `cperl6-toggle-autohelp' implemented.
;;;  `while SPACE LESS' was buggy.
;;;  `-text' in `[-text => 1]' was not highlighted.
;;;  `cperl6-after-block-p' was FALSE after `sub f {}'.

;;;; After 2.5:
;;;  `foreachmy', `formy' expanded too.
;;;  Expand `=pod-directive'.
;;;  `cperl6-linefeed' behaves reasonable in POD-directive lines.
;;;  `cperl6-electric-keyword' prints a message, governed by
;;;    `cperl6-message-electric-keyword'.
...
;;;  `cperl6-indent-region-fix-else' implemented.
;;;  `cperl6-fix-line-spacing' implemented.
;;;  `cperl6-invert-if-unless' implemented (C-c C-t and in Menu).
;;;  Upgraded hints to mention 20.2's goods/bads.
;;;  Started to use `cperl6-extra-newline-before-brace-multiline',
;;;    `cperl6-break-one-line-blocks-when-indent',
;;;    `cperl6-fix-hanging-brace-when-indent', `cperl6-merge-trailing-else'.

;;;; After 2.9:
;;;  Workaround for another `font-lock's `syntax-table' text-property bug.
;;;  `zerop' could be applied to nil.
;;;  At last, may work with `font-lock' without setting `cperl6-font-lock'.
;;;    (We expect that starting from 19.33, `font-lock' supports keywords
;;;     being a function - what is a correct version?)
;;;  Rename `cperl6-indent-region-fix-else' to
;;;    `cperl6-indent-region-fix-constructs'.
;;;  `cperl6-fix-line-spacing' could be triggered inside strings, would not
;;;     know what to do with BLOCKs of map/printf/etc.
;;;  `cperl6-merge-trailing-else' and `cperl6-fix-line-spacing' handle
;;;     `continue' too.
...
;;;    Switched on by `cperl6-syntaxify-by-font-lock', off by default so far.

;;;; After 2.12:
;;;  Remove some commented out chunks.
;;;  Styles are slightly updated (a lot of work is needed, especially
;;;    with new `cperl6-fix-line-spacing').
...
;;;  `cperl6-praise' updated.
;;;  `cperl6-toggle-construct-fix' added on C-c C-w and menu.
...
;;;; After 3.1:
;;;  Fixed customization to honor cperl6-hairy.
;;;  Created customization groups.  Sent to RMS to include into 2.3.

;;;; After 3.2:
;;;  Interaction of `font-lock-hot-pass' and `cperl6-syntaxify-by-font-lock'.
;;;  (`cperl6-after-block-and-statement-beg'):
;;;  (`cperl6-after-block-p'):
;;;  (`cperl6-after-expr-p'):	It is BLOCK if we reach lim when backup sexp.
;;;  (`cperl6-indent-region'):	Make a marker for END - text added/removed.
;;;  (`cperl6-style-alist', `cperl6-styles-entries')
;;;		Include `cperl6-merge-trailing-else' where the value is clear.

;;;; After 3.3:
;;;  (`cperl6-tips'):
;;;  (`cperl6-problems'):	Improvements to docs.

;;;; After 3.4:
;;;  (`cperl6-mode'):		Make lazy syntaxification possible.
;;;  (`cperl6-find-pods-heres'): Safe a position in buffer where it is safe to
;;;				restart syntaxification.
;;;  (`cperl6-syntaxify-by-font-lock'): Set to t, should be safe now.

;;;; After 3.5:
;;;  (`cperl6-syntaxify-by-font-lock'): Better default, customizes to
;;;				`message' too.

;;;; After 3.6:
;;;  (`cperl6-find-pods-heres'): changed so that -d ?foo? is a RE.
;;;  (`cperl6-array-face'): changed name from `font-lock-emphasized-face'.
;;;  (`cperl6-hash-face'): changed name from  `font-lock-other-emphasized-face'.
...
;;;  (73 vs 15 with imenu).
;;;  (`cperl6-emacs-can-parse'):	New state.
;;;  (`cperl6-indent-line'):	Corrected to use global state.
;;;  (`cperl6-calculate-indent'):	Likewise.
;;;  (`cperl6-fix-line-spacing'):	Likewise (not used yet).

;;;; After 3.8:
;;;  (`cperl6-choose-color'):	Converted to a function (to be compilable in text-mode).

;;;; After 3.9:
;;;  (`cperl6-dark-background '):	Disable without window-system.

;;;; After 3.10:
;;;  Do `defface' only if window-system.

;;;; After 3.11:
;;;  (`cperl6-fix-line-spacing'):	sped up to bail out early.
;;;  (`cperl6-indent-region'):	Disable hooks during the call (how to call them later?).
...
;;;Function Name                              Call Count  Elapsed Time  Average Time
;;;=========================================  ==========  ============  ============
;;;cperl6-indent-exp                           1           10.039999999  10.039999999
;;;cperl6-indent-region                        1           10.0          10.0
;;;cperl6-indent-line                          821         6.2100000000  0.0075639464
;;;cperl6-calculate-indent                     821         5.0199999999  0.0061144945
;;;cperl6-backward-to-noncomment               2856        2.0500000000  0.0007177871
;;;cperl6-fontify-syntaxically                 2           1.78          0.8900000000
;;;cperl6-find-pods-heres                      2           1.78          0.8900000000
;;;cperl6-update-syntaxification               1           1.78          1.78
;;;cperl6-fix-line-spacing                     769         1.4800000000  0.0019245773
;;;cperl6-after-block-and-statement-beg        163         1.4100000000  0.0086503067
;;;cperl6-block-p                              775         1.1800000000  0.0015225806
;;;cperl6-to-comment-or-eol                    3652        1.1200000000  0.0003066812
;;;cperl6-after-block-p                        165         1.0500000000  0.0063636363
;;;cperl6-commentify                           141         0.22          0.0015602836
;;;cperl6-get-state                            813         0.16          0.0001968019
;;;cperl6-backward-to-start-of-continued-exp   26          0.12          0.0046153846
;;;cperl6-delay-update-hook                    2107        0.0899999999  4.271...e-05
;;;cperl6-protect-defun-start                  141         0.0700000000  0.0004964539
;;;cperl6-after-label                          407         0.0599999999  0.0001474201
;;;cperl6-forward-re                           139         0.0299999999  0.0002158273
;;;cperl6-comment-indent                       26          0.0299999999  0.0011538461
;;;cperl6-use-region-p                         8           0.0           0.0
;;;cperl6-lazy-hook                            15          0.0           0.0
;;;cperl6-after-expr-p                         8           0.0           0.0
;;;cperl6-font-lock-unfontify-region-function  1           0.0           0.0

;;;Function Name                              Call Count  Elapsed Time  Average Time
;;;=========================================  ==========  ============  ============
;;;cperl6-fix-line-spacing                     769         1.4500000000  0.0018855656
;;;cperl6-indent-line                          13          0.3100000000  0.0238461538
;;;cperl6-after-block-and-statement-beg        69          0.2700000000  0.0039130434
;;;cperl6-after-block-p                        69          0.2099999999  0.0030434782
;;;cperl6-calculate-indent                     13          0.1000000000  0.0076923076
;;;cperl6-backward-to-noncomment               177         0.0700000000  0.0003954802
;;;cperl6-get-state                            13          0.0           0.0
;;;cperl6-to-comment-or-eol                    179         0.0           0.0
;;;cperl6-get-help-defer                       1           0.0           0.0
;;;cperl6-lazy-hook                            11          0.0           0.0
;;;cperl6-after-expr-p                         2           0.0           0.0
;;;cperl6-block-p                              13          0.0           0.0
;;;cperl6-after-label                          5           0.0           0.0

;;;; After 3.12:
;;;  (`cperl6-find-pods-heres'): do not warn on `=cut' if doing a chunk only.

;;;; After 3.13:
;;;  (`cperl6-mode'): load pseudo-faces on `cperl6-find-pods-heres' (for 19.30).
;;;  (`x-color-defined-p'): was not compiling on XEmacs
;;;  (`cperl6-find-pods-heres'): 1 << 6 was OK, but 1<<6 was considered as HERE
;;;                             <file/glob> made into a string.

;;;; After 3.14:
;;;  (`cperl6-find-pods-heres'): Postpone addition of faces after syntactic step
;;;				Recognition of <FH> was wrong.
;;;  (`cperl6-clobber-lisp-bindings'): if set, C-c variants are the old ones
;;;  (`cperl6-unwind-to-safe'):	New function.
;;;  (`cperl6-fontify-syntaxically'): Use `cperl6-unwind-to-safe' to start at reasonable position.

;;;; After 3.15:
;;;  (`cperl6-forward-re'):	Highlight the trailing / in s/foo// as string.
;;;			Highlight the starting // in s//foo/ as function-name.

;;;; After 3.16:
;;;  (`cperl6-find-pods-heres'): Highlight `gem' in s///gem as a keyword.

;;;; After 4.0:
;;;  (`cperl6-find-pods-heres'): `qr' added
;;;  (`cperl6-electric-keyword'):	Likewise
;;;  (`cperl6-electric-else'):		Likewise
;;;  (`cperl6-to-comment-or-eol'):	Likewise
;;;  (`cperl6-make-regexp-x'):	Likewise
;;;  (`cperl6-init-faces'):	Likewise, and `lock' (as overridable?).
;;;  (`cperl6-find-pods-heres'): Knows that split// is null-RE.
;;;				Highlights separators in 3-parts expressions
;;;				as labels.

;;;; After 4.1:
;;;  (`cperl6-find-pods-heres'):	<> was considered as a glob
;;;  (`cperl6-syntaxify-unwind'): New configuration variable
;;;  (`cperl6-fontify-m-as-s'):	New configuration variable

;;;; After 4.2:
;;;  (`cperl6-find-pods-heres'): of the last line being `=head1' fixed.
...
;;;  (`cperl6-unwind-to-safe'):	would not go to beginning of buffer.
;;;  (`cperl6-electric-pod'):	check for after-expr was performed
;;;				inside of POD too.

;;;; After 4.3:
;;;  (`cperl6-backward-to-noncomment'):	better treatment of PODs and HEREs.

;;;  Indent-line works good, but indent-region does not - at toplevel...
;;;  (`cperl6-unwind-to-safe'):	Signature changed.
;;;  (`x-color-defined-p'):     was defmacro'ed with a tick.  Remove another def.
;;;  (`cperl6-clobber-mode-lists'): New configuration variable.
;;;  (`cperl6-array-face'): One of definitions was garbled.

;;;; After 4.4:
;;;  (`cperl6-not-bad-style-regexp'):	Updated.
;;;  (`cperl6-make-regexp-x'):	Misprint in a message.
;;;  (`cperl6-find-pods-heres'):	$a-1 ? foo : bar; was a regexp.
;;;                             `<< (' was considered a start of POD.
;;;  Init:			`cperl6-is-face' was busted.
;;;  (`cperl6-make-face'):	New macros.
;;;  (`cperl6-force-face'):	New macros.
;;;  (`cperl6-init-faces'):	Corrected to use new macros;
;;;				`if' for copying `reference-face' to
;;;				`constant-face' was backward.
;;;  (`font-lock-other-type-face'): Done via `defface' too.

;;;; After 4.5:
;;;  (`cperl6-init-faces-weak'):	use `cperl6-force-face'.
;;;  (`cperl6-after-block-p'):	After END/BEGIN we are a block.
;;;  (`cperl6-mode'):		`font-lock-unfontify-region-function'
;;;				was set to a wrong function.
;;;  (`cperl6-comment-indent'):	Commenting __END__ was not working.
;;;  (`cperl6-indent-for-comment'):	Likewise.
;;;				(Indenting is still misbehaving at toplevel.)

;;;; After 4.5:
;;;  (`cperl6-unwind-to-safe'):	Signature changed, unwinds end too.
;;;  (`cperl6-find-pods-heres'):	mark qq[]-etc sections as syntax-type=string
;;;  (`cperl6-fontify-syntaxically'): Unwinds start and end to go out of
;;;				     long strings (not very successful).

;;;   >>>>  Cperl6 should be usable in write mode too now <<<<

;;;  (`cperl6-syntaxify-by-font-lock'): Better default - off in text-mode.
;;;  (`cperl6-tips'): 		Updated docs.
;;;  (`cperl6-problems'):	Updated docs.

;;;; After 4.6:
;;;  (`cperl6-calculate-indent'):	Did not consider `,' as continuation mark for statements.
;;;  (`cperl6-write-tags'):	Correct for XEmacs's `visit-tags-table-buffer'.

;;;; After 4.7:
;;;  (`cperl6-calculate-indent'): Avoid parse-data optimization at toplevel.
;;;				 Should indent correctly at toplevel too.
;;;  (`cperl6-tags-hier-init'):	Gross hack to pretend we work (are we?).
;;;  (`cperl6-find-pods-heres'):	Was not processing sub protos after a comment ine.
;;;				Was treating $a++ <= 5 as a glob.

;;;; After 4.8:
;;;  (toplevel):		require custom unprotected => failure on 19.28.
;;;  (`cperl6-xemacs-p')		defined when compile too
;;;  (`cperl6-tags-hier-init'):	Another try to work around XEmacs problems
;;;				Better progress messages.
;;;  (`cperl6-find-tags'):	Was writing line/pos in a wrong order,
;;;				pos off by 1 and not at beg-of-line.
;;;  (`cperl6-etags-snarf-tag'): New macro
;;;  (`cperl6-etags-goto-tag-location'): New macro
;;;  (`cperl6-write-tags'):	When removing old TAGS info was not
;;;				relativizing filename

;;;; After 4.9:
;;;  (`cperl6-version'):		New variable.  New menu entry

;;;; After 4.10:
;;;  (`cperl6-tips'):		Updated.
;;;  (`cperl6-non-problems'):	Updated.
;;;  random:			References to future 20.3 removed.

;;;; After 4.11:
;;;  (`perl-font-lock-keywords'): Would not highlight `sub foo($$);'.
;;;  Docstrings:		Menu was described as `Cperl6' instead of `Perl'

;;;; After 4.12:
;;;  (`cperl6-toggle-construct-fix'): Was toggling to t instead of 1.
;;;  (`cperl6-ps-print-init'):	Associate `cperl6-array-face', `cperl6-hash-face'
...
;;;				Use `eval-after-load'.
;;;  (`cperl6-init-faces'):	remove init `font-lock-other-emphasized-face'.
;;;				remove init `font-lock-emphasized-face'.
;;;				remove init `font-lock-keyword-face'.
;;;  (`cperl6-tips-faces'):	New variable and an entry into Mini-docs.
;;;  (`cperl6-indent-region'):	Do not indent whitespace lines
;;;  (`cperl6-indent-exp'):	Was not processing else-blocks.
;;;  (`cperl6-calculate-indent'): Remove another parse-data optimization
;;;				 at toplevel: would indent correctly.
;;;  (`cperl6-get-state'):	NOP line removed.

;;;; After 4.13:
;;;  (`cperl6-ps-print-init'):	Remove not-Cperl6-related faces.
;;;  (`cperl6-ps-print'):	New function and menu entry.
;;;  (`cperl6-ps-print-face-properties'):	New configuration variable.
;;;  (`cperl6-invalid-face'):	New configuration variable.
;;;  (`cperl6-nonoverridable-face'):	New face.  Renamed from
;;;					`font-lock-other-type-face'.
;;;  (`perl-font-lock-keywords'):	Highlight trailing whitespace
;;;  (`cperl6-contract-levels'):	Documentation corrected.
;;;  (`cperl6-contract-level'):	Likewise.

;;;; After 4.14:
;;;  (`cperl6-ps-print'): `ps-print-face-extension-alist' was not in old Emaxen,
;;;				same with `ps-extend-face-list'
;;;  (`cperl6-ps-extend-face-list'):	New macro.

;;;; After 4.15:
;;;  (`cperl6-init-faces'):	Interpolate `cperl6-invalid-face'.
;;;  (`cperl6-forward-re'):	Emit a meaningful error instead of a cryptic
;;;				one for uncomplete REx near end-of-buffer.
;;;  (`cperl6-find-pods-heres'):	Tolerate unfinished REx at end-of-buffer.

;;;; After 4.16:
;;;  (`cperl6-find-pods-heres'): `unwind-protect' was left commented.

;;;; After 4.17:
;;;  (`cperl6-invalid-face'):	Change to ''underline.

;;;; After 4.18:
;;;  (`cperl6-find-pods-heres'):	/ and ? after : start a REx.
;;;  (`cperl6-after-expr-p'):	Skip labels when checking
;;;  (`cperl6-calculate-indent'): Correct for labels when calculating
...
;;;; After 4.20:
;;;  (`cperl6-tips'):		Another workaround added.  Sent to RMS for 20.4.

;;;; After 4.21:
;;;  (`cperl6-praise'):		Mention linear-time indent.
;;;  (`cperl6-find-pods-heres'):	@if ? a : b was considered a REx.

;;;; After 4.22:
;;;  (`cperl6-after-expr-p'):	Make true after __END__.
;;;  (`cperl6-electric-pod'):	"SYNOPSIS" was misspelled.

;;;; After 4.23:
;;;  (`cperl6-beautify-regexp-piece'):	Was not allowing for *? after a class.
...
;;;  (`cperl6-beautify-regexp'):	Add an optional count argument to go that
;;;				many levels deep.
;;;  (`cperl6-beautify-level'):	Likewise
;;;  Menu:			Add new entries to Regexp menu to do one level
;;;  (`cperl6-contract-level'):	Was entering an infinite loop
;;;  (`cperl6-find-pods-heres'):	Typo (double quoting).
;;;				Was detecting < $file > as FH instead of glob.
;;;				Support for comments in RExen (except
;;;				for m#\#comment#x), governed by
;;;				`cperl6-regexp-scan'.
;;;  (`cperl6-regexp-scan'):	New customization variable.
;;;  (`cperl6-forward-re'):	Improve logic of resetting syntax table.

;;;; After 4.23 and: After 4.24:
;;;  (`cperl6-contract-levels'):	Restore position.
;;;  (`cperl6-beautify-level'):	Likewise.
;;;  (`cperl6-beautify-regexp'):	Likewise.
;;;  (`cperl6-commentify'):	Rudimental support for length=1 runs
;;;  (`cperl6-find-pods-heres'):	Process 1-char long REx comments too /a#/x
...
;;;				  present in `cperl6-indent-region' eons ago.)
;;; To reproduce:
...
;;;
;;; In Cperl6 it was triggered by inserting then deleting `/' at start of
;;;      /  a (?# asdf  {[(}asdf )ef,/;

;;;; After 4.25:
;;; (`cperl6-commentify'):	Was recognizing length=2 "strings" as length=1.
;;; (`imenu-example--create-perl-index'):
;;;				Was not enforcing syntaxification-to-the-end.
;;; (`cperl6-invert-if-unless'):	Allow `for', `foreach'.
;;; (`cperl6-find-pods-heres'):	Quote `cperl6-nonoverridable-face'.
;;;				Mark qw(), m()x as indentable.
;;; (`cperl6-init-faces'):	Highlight `sysopen' too.
;;;				Highlight $var in `for my $var' too.
;;; (`cperl6-invert-if-unless'):	Was leaving whitespace at end.
;;; (`cperl6-linefeed'):		Was splitting $var{$foo} if point after `{'.
;;; (`cperl6-calculate-indent'): Remove old commented out code.
;;;				Support (primitive) indentation of qw(), m()x.


;;;; After 4.26:
;;; (`cperl6-problems'):		Mention `fill-paragraph' on comment. \"" and
;;;				q [] with intervening newlines.
;;; (`cperl6-autoindent-on-semi'):	New customization variable.
;;; (`cperl6-electric-semi'):	Use `cperl6-autoindent-on-semi'.
;;; (`cperl6-tips'):		Mention how to make Cperl6 the default mode.
;;; (`cperl6-mode'):		Support `outline-minor-mode'
;;;				(Thanks to Mark A. Hershberger).
;;; (`cperl6-outline-level'):	New function.
;;; (`cperl6-highlight-variables-indiscriminately'):	New customization var.
;;; (`cperl6-init-faces'):	Use `cperl6-highlight-variables-indiscriminately'.
;;;				(Thanks to Sean Kamath <kamath@pogo.wv.tek.com>).
;;; (`cperl6-after-block-p'):	Support CHECK and INIT.
;;; (`cperl6-init-faces'):	Likewise and "our".
;;;				(Thanks to Doug MacEachern <dougm@covalent.net>).
;;; (`cperl6-short-docs'):	Likewise and "our".


;;;; After 4.27:
;;; (`cperl6-find-pods-heres'):	Recognize \"" as a string.
;;;				Mark whitespace and comments between q and []
;;;				  as `syntax-type' => `prestring'.
;;;				Allow whitespace between << and "FOO".
;;; (`cperl6-problems'):		Remove \"" and q [] with intervening newlines.
;;;				Mention multiple <<EOF as unsupported.
;;; (`cperl6-highlight-variables-indiscriminately'):	Doc misprint fixed.
;;; (`cperl6-indent-parens-as-block'):	New configuration variable.
;;; (`cperl6-calculate-indent'):	Merge cases of indenting non-BLOCK groups.
;;;				Use `cperl6-indent-parens-as-block'.
;;; (`cperl6-find-pods-heres'):	Test for =cut without empty line instead of
;;;				complaining about no =cut.
;;; (`cperl6-electric-pod'):	Change the REx for POD from "\n\n=" to "^\n=".
;;; (`cperl6-find-pods-heres'):	Likewise.
;;; (`cperl6-electric-pod'):	Change `forward-sexp' to `forward-word':
;;;				POD could've been marked as comment already.
;;; (`cperl6-unwind-to-safe'):	Unwind before start of POD too.

;;;; After 4.28:
;;; (`cperl6-forward-re'):	Throw an error at proper moment REx unfinished.

;;;; After 4.29:
;;; (`x-color-defined-p'):	Make an extra case to peacify the warning.
;;; Toplevel:			`defvar' to peacify the warnings.
;;; (`cperl6-find-pods-heres'):	Could access `font-lock-comment-face' in -nw.
;;;;				No -nw-compile time warnings now.
;;; (`cperl6-find-tags'):	TAGS file had too short substring-to-search.
;;;				Be less verbose in non-interactive mode
;;; (`imenu-example--create-perl-index'):	Set index-marker after name
;;; (`cperl6-outline-regexp'):	New variable.
;;; (`cperl6-outline-level'):	Made compatible with `cperl6-outline-regexp'.
;;; (`cperl6-mode'):		Made use `cperl6-outline-regexp'.

;;;; After 4.30:
;;; (`cperl6-find-pods-heres'):	=cut the last thing, no blank line, was error.
;;; (`cperl6-outline-level'):	Make start-of-file same level as `package'.

;;;; After 4.31:
;;; (`cperl6-electric-pod'):	`head1' and `over' electric only if empty.
;;; (`cperl6-unreadable-ok'):	New variable.
;;; (`cperl6-find-tags'):	Use `cperl6-unreadable-ok', do not fail
;;;				on an unreadable file
;;; (`cperl6-write-tags'):	Use `cperl6-unreadable-ok', do not fail
...
;;;  (`cperl6-noscan-files-regexp'): Do not scan CVS subdirs
;;;  (`cperl6-problems'):	Note that newer XEmacsen may syntaxify too
;;;  (`imenu-example--create-perl-index'):
;;;				Renamed to `cperl6-imenu--create-perl-index'
;;;  (`cperl6-mode'):		Replace `make-variable-buffer-local' by `make-local-variable'
;;;  (`cperl6-setup-tmp-buf'):	Likewise
;;;  (`cperl6-fix-line-spacing'): Fix a misprint of "t" for "\t"
;;;  (`cperl6-next-bad-style'):  Fix misprints in character literals

;;;; After 4.33:
;;;  (`cperl6-font-lock-keywords'): +etc: Aliased to perl-font-lock-keywords.

;;;; After 4.34:
;;;  Further updates of whitespace and spelling w.r.t. RMS version.
;;;  (`cperl6-font-lock-keywords'): +etc: Avoid warnings when aliasing.
;;;  (`cperl6-mode'):		Use `normal-auto-fill-function' if present.
;;;  (`cperl6-use-major-mode'):	New variable
;;;  (`cperl6-can-font-lock'):	New variable; replaces `window-system'
;;;  (`cperl6-tags-hier-init'):	use `display-popup-menus-p' (if present)
;;;				to choose `x-popup-menu' vs `tmm-prompt'

;;;; 4.35 has the following differences from version 1.40+ of RMS Emacs:

;;; New variables `cperl6-use-major-mode', `cperl6-can-font-lock';
;;; `cperl6-use-major-mode' is (effectively) 'cperl6-mode in RMS.
;;; `cperl6-under-as-char'  is nil in RMS.
;;; Minor differences in docstrings, and `cperl6-non-problems'.
;;; Backward compatibility addressed: (`); (function (lambda ...)); font-lock;
;;; (:italic t bold t) vs (:slant italic :weight bold) in faces;
;;; `normal-auto-fill-function'.
;;; RMS version has wrong logic in `cperl6-calculate-indent': $a = { } is
...
;;; in `cperl6-find-pods-heres'. [Cosmetic]

;;;; After 4.35:
;;;  (`cperl6-find-pods-heres'):	If no end of HERE-doc found, mark to the end
...
;;;  (`cperl6-perldoc'):		Use case-sensitive search (contributed).
;;;  (`cperl6-fix-line-spacing'): Allow "_" in $vars of foreach etc. when
;;;				underscore isn't a word char (gdj-contributed).
;;;  (`defun-prompt-regexp'):	Allow prototypes.
;;;  (`cperl6-vc-header-alist'):	Extract numeric version from the Id.
;;;  Toplevel:			Put toggle-autohelp into the mode menu.
;;;				Better docs for toggle/set/unset autohelp.
;;;  (`cperl6-electric-backspace-untabify'): New customization variable
;;;  (`cperl6-after-expr-p'):	Works after here-docs, formats, and PODs too
;;;				(affects many electric constructs).
;;;  (`cperl6-calculate-indent'): Takes into account `first-format-line' ==>
;;;				works after format.
;;;  (`cperl6-short-docs'):	Make it work with ... too.
;;;				"array context" ==> "list context"
;;;  (`cperl6-electric-keyword'): make $if (etc: "$@%&*") non-electric
;;;				'(' after keyword would insert a doubled paren
;;;  (`cperl6-electric-paren'):	documented affected by `cperl6-electric-parens'
;;;  (`cperl6-electric-rparen'):	Likewise
;;;  (`cperl6-build-manpage'):	New function by Nick Roberts
;;;  (`cperl6-perldoc'):		Make it work in XEmacs too

;;;; After 4.36:
;;;  (`cperl6-find-pods-heres'):	Recognize s => 1 and {s} (as a key or varname),
;;;				{ s:: } and { s::bar::baz } as varnames.
;;;  (`cperl6-after-expr-p'):	Updates syntaxification before checks
;;;  (`cperl6-calculate-indent'): Likewise
;;;				Fix wrong indent of blocks starting with POD
;;;  (`cperl6-after-block-p'):	Optional argument for checking for a pre-block
;;;				Recognize `continue' blocks too.
;;;  (`cperl6-electric-brace'):	use `cperl6-after-block-p' for detection;
;;;				Now works for else/continue/sub blocks
;;;  (`cperl6-short-docs'):	Minor edits; make messages fit 80-column screen

;;;; After 5.0:
;;;  `cperl6-add-tags-recurse-noxs-fullpath': new function (for -batch mode)
...
;;;;;;    (The exception is `cperl6-etags' - but it is not used in the rest
;;;;;;    of the mode.)
...
;;; `cperl6-hook-after-change':	New configuration variable
;;; `cperl6-vc-sccs-header':	Likewise
;;; `cperl6-vc-sccs-header':	Likewise
;;; `cperl6-vc-header-alist':	Default via two preceding variables
;;; `cperl6-invalid-face':	Remove double quoting under XEmacs
;;;					(still needed under 21.2)
;;; `cperl6-tips':		Update URLs for resources
;;; `cperl6-problems':		Likewise
;;; `cperl6-praise':		Mention new features
;;; New C-c key bindings:	for `cperl6-find-bad-style',
;;;	`cperl6-pod-spell', `cperl6-here-doc-spell', `cperl6-narrow-to-here-doc',
;;;	`cperl6-perdoc', `cperl6-perldoc-at-point'
;;; Cperl6 Mode menu changes:	"Fix style by spaces", "Imenu on Perl Info"
;;;	moved, new submenu of Tools with Ispell entries and narrowing.
;;; `cperl6-after-sub-regexp':	New defsubst
;;; `cperl6-imenu--function-name-regexp-perl': Use `cperl6-after-sub-regexp',
;;;				Allows heads up to head4
;;;				Allow "package;"
;;; `defun-prompt-regexp':	Use `cperl6-after-sub-regexp',
;;; `paren-backwards-message':	??? Something for XEmacs???
;;; `cperl6-mode':		Never auto-switch abbrev-mode off
;;;				Try to allow '_' be non-word char
;;;				Do not use `font-lock-unfontify-region-function' on XEmacs
;;;				Reset syntax cache on mode start
;;;				Support multiline facification (even
;;;				        on legacy `font-lock')
;;; `cperl6-facemenu-add-face-function':	??? Some contributed code ???
;;; `cperl6-after-change-function':	Since `font-lock' and `lazy-lock'
;;;         refuse to inform us whether the fontification is due to lazy
;;;         calling or due to edit to a buffer, install our own hook
;;;         (controlled by `cperl6-hook-after-change')
;;; `cperl6-electric-pod':	=cut may have been recognized as start
;;; `cperl6-block-p':		Moved, updated for attributes
;;; `cperl6-calculate-indent':	Try to allow '_' be non-word char
;;;				Support subs with attributes
;;; `cperl6-where-am-i':		Queit (?) a warning
;;; `cperl6-cached-syntax-table'	New function
;;; `cperl6-forward-re':		Use `cperl6-cached-syntax-table'
;;; `cperl6-unwind-to-safe':	Recognize `syntax-type' property
;;;					changing in a middle of line
;;; `cperl6-find-sub-attrs':	New function
;;; `cperl6-find-pods-heres':	Allow many <<EOP per line
...
;;;				        not recognized
;;;				Better comments
;;; `cperl6-after-block-p':	Remarks on diff with `cperl6-block-p'
;;;				Allow subs with attributes, labels
;;;				Do not confuse "else::foo" with "else"
;;;				Minor optimizations...
;;; `cperl6-after-expr-p':	Try to allow '_' be non-word char
;;; `cperl6-fill-paragraph':	Try to detect a major bug in Emacs
;;;         with `looking-at' inside `narrow' and bulk out if found
;;; `cperl6-imenu--create-perl-index':	Updates for new
;;;         `cperl6-imenu--function-name-regexp-perl'
;;; `cperl6-outline-level':	Likewise
;;; `cperl6-init-faces':		Allow multiline subroutine headers
;;;         and my/our declarations, and ones with comments
;;;				Allow subroutine attributes
;;; `cperl6-imenu-on-info':	Better docstring.
;;; `cperl6-etags'		Rudimentary support for attributes
;;;				Support for packages and "package;"
;;; `cperl6-add-tags-recurse-noxs':	Better (?) docstring
;;; `cperl6-add-tags-recurse-noxs-fullpath': Likewise
;;; `cperl6-tags-hier-init':	Misprint for `fboundp' fixed
;;; `cperl6-not-bad-style-regexp':	Try to allow '_' be non-word char
;;; `cperl6-perldoc':		Add autoload
;;; `cperl6-perldoc-at-point':	Likewise
;;; `cperl6-here-doc-spell':	New function
;;; `cperl6-pod-spell':		Likewise
;;; `cperl6-map-pods-heres':	Likewise
;;; `cperl6-get-here-doc-region':	Likewise
;;; `cperl6-font-lock-fontify-region-function': Likewise (backward compatibility
;;;					        for legacy `font-lock')
;;; `cperl6-font-lock-unfontify-region-function': Fix style
;;; `cperl6-fontify-syntaxically':	Recognize and optimize away
;;;         deferred calls with no-change.  Governed by `cperl6-hook-after-change'
;;; `cperl6-fontify-update':	Recognize that syntaxification region
;;;         can be larger than fontification one.
;;;         XXXX we leave `cperl6-postpone' property, so this is quadratic...
;;; `cperl6-fontify-update-bad':	Temporary placeholder until
;;;         it is clear how to implement `cperl6-fontify-update'.
;;; `cperl6-time-fontification':	New function
;;; `attrib-group':		New text attribute
;;; `multiline':		New value: `syntax-type' text attribute

;;;; After 5.2:
;;; `cperl6-emulate-lazy-lock':	New function
;;; `cperl6-fontify-syntaxically': Would skip large regions
;;; Add `cperl6-time-fontification', `cperl6-emulate-lazy-lock' to menu
;;; Some globals were declared, but uninitialized

;;;; After 5.3, 5.4:
;;; `cperl6-facemenu-add-face-function':	Add docs, fix U<>
;;; Copyright message updated.
;;; `cperl6-init-faces':		Work around a bug in `font-lock'. May slow
...
;;; After 5.5, 5.6
;;; `cperl6-fontify-syntaxically': after-change hook could reset
;;;	`cperl6-syntax-done-to' to a middle of line; unwind to BOL.

;;; After 5.7:
;;; `cperl6-init-faces':		Allow highlighting of local ($/)
;;; `cperl6-problems-old-emaxen': New variable (for the purpose of DOCSTRING).
;;; `cperl6-problems':		Remove fixed problems.
;;; `cperl6-find-pods-heres':	Recognize #-comments in m##x too
;;;				Recognize charclasses (unless delimiter is \).
;;; `cperl6-fontify-syntaxically': Unwinding to safe was done in wrong order
;;; `cperl6-regexp-scan':	Update docs
;;; `cperl6-beautify-regexp-piece': use information got from regexp scan
...
;;;
;;; `cperl6-find-pods-heres':	Recognize POSIX classes in REx charclasses
...
;;; `cperl6-calculate-indent':	Indents //x -RExen accordning to parens level
;;; `cperl6-to-comment-or-eol':	Recognize ends of `syntax-type' constructs
;;; `cperl6-backward-to-noncomment': Recognize stringy `syntax-type' constructs
;;;				Support `narrow'ed buffers.
;;; `cperl6-praise':		Remove a reservation
;;; `cperl6-make-indent':	New function
;;; `cperl6-indent-for-comment':	Use `cperl6-make-indent'
;;; `cperl6-indent-line':	Likewise
;;; `cperl6-lineup':		Likewise
;;; `cperl6-beautify-regexp-piece': Likewise
;;; `cperl6-contract-level':	Likewise
;;; `cperl6-toggle-set-debug-unwind': New function
;;;				New menu entry for this
;;; `fill-paragraph-function':	Use when `boundp'
;;; `cperl6-calculate-indent':	Take into account groups when indenting RExen
;;; `cperl6-to-comment-or-eol':	Recognize # which end a string
;;; `cperl6-modify-syntax-type':	Make only syntax-table property non-sticky
;;; `cperl6-fill-paragraph':	Return t: needed for `fill-paragraph-function'
;;; `cperl6-fontify-syntaxically': More clear debugging message
;;; `cperl6-pod2man-build-command': XEmacs portability: check `Man-filter-list'
;;; `cperl6-init-faces':		More complicated highlight even on XEmacs (new)
;;; Merge cosmetic changes from XEmacs

;;; After 5.9:
;;; `cperl6-1+':			Moved to before the first use
;;; `cperl6-1-':			Likewise
...
;;; `cperl6-font-locking':	New internal variable
;;; `cperl6-beginning-of-property': New function
;;; `cperl6-calculate-indent':	Use `cperl6-beginning-of-property'
;;;	instead of `previous-single-property-change'
;;; `cperl6-unwind-to-safe':	Likewise
;;; `cperl6-after-expr-p':	Likewise
;;; `cperl6-get-here-doc-region': Likewise
;;; `cperl6-font-lock-fontify-region-function': Likewise
;;; `cperl6-to-comment-or-eol':	Do not call `cperl6-update-syntaxification'
;;;					recursively
;;;				Bound `next-single-property-change'
;;;					via `point-max'
;;; `cperl6-unwind-to-safe':	Bound likewise
;;; `cperl6-font-lock-fontify-region-function': Likewise
;;; `cperl6-find-pods-heres':	Mark as recursive for `cperl6-to-comment-or-eol'
;;;				Initialization of
;;;	`cperl6-font-lock-multiline-start' could be missed if the "main"
;;;	fontification did not run due to the keyword being already fontified.
;;; `cperl6-pod-spell':		Return t from do-one-chunk function
;;; `cperl6-map-pods-heres':	Stop when the worker returns nil
;;;				Call `cperl6-update-syntaxification'
;;; `cperl6-get-here-doc-region': Call `cperl6-update-syntaxification'
;;; `cperl6-get-here-doc-delim':	Remove unused function

;;; After 5.11:

;;;  The possible lockup of Emacs (introduced in 5.10) fixed

;;; `cperl6-unwind-to-safe':	`cperl6-beginning-of-property' won't return nil
;;; `cperl6-syntaxify-for-menu':	New customization variable
;;; `cperl6-select-this-pod-or-here-doc': New function
;;; `cperl6-get-here-doc-region': Extra argument.
;;;				Do not adjust pos by 1.

;;; New menu entries (Perl/Tools): selection of current POD or HERE-DOC section
;;;				(Debugging Cperl6:) backtrace on fontification

;;; After 5.12:
;;; `cperl6-cached-syntax-table': use `car-safe'
;;; `cperl6-forward-re':		Remove spurious argument SET-ST
;;;				Add documentation
;;; `cperl6-forward-group-in-re': New function
;;; `cperl6-find-pods-heres':	Find and highlight (?{}) blocks in RExen.
;;;	(XXXX Temporary (?) hack is to syntax-mark them as comment)

;;; After 5.13:
;;; `cperl6-string-syntax-table': Make { and } not-grouping
;;;   (Sometimes they ARE grouping in RExen, but matching them would only
;;;    confuse in many situations when they are not).
;;; `beginning-of-buffer':	Replaced two occurences with goto-char...
;;; `cperl6-calculate-indent':	`char-after' could be nil...
;;; `cperl6-find-pods-heres':	REx can start after "[" too
;;;				Hightlight (??{}) in RExen too
;;; `cperl6-maybe-white-and-comment-rex': New constant
;;; `cperl6-white-and-comment-rex': Likewise
;;;				XXXX Not very efficient, but hard to make
;;;				better while keeping 1 group.

;;; After 5.13:
;;; `cperl6-find-pods-heres':	$foo << identifier() is not a HERE-DOC
;;;				Likewise for 1 << identifier

;;; After 5.14:
;;; `cperl6-find-pods-heres':	Different logic for $foo .= <<EOF etc
;;;				Error-less condition-case could fail
;;; `cperl6-font-lock-fontify-region-function': Likewise
;;; `cperl6-init-faces':		Likewise

;;; After 5.15:
;;; `cperl6-find-pods-heres':	Support property REx-part2
;;; `cperl6-calculate-indent':	Likewise
;;;				Don't special-case REx with non-empty 1st line
;;; `cperl6-find-pods-heres':	In RExen, highlight non-literal backslashes
;;;				Invert highlighting of charclasses:
;;;					now the envelop is highlighted
;;;				Highlight many others 0-length builtins
;;; `cperl6-praise':		Mention indenting and highlight in RExen.

;;; After 5.15:
;;; `cperl6-find-pods-heres':	Highlight capturing parens in REx

;;; After 5.16:
;;; `cperl6-find-pods-heres':	Highlight '|' for alternation
;;;	Initialize `font-lock-warning-face' if not present
;;; `cperl6-find-pods-heres':	Use `font-lock-warning-face' instead of
;;;					 `font-lock-function-name-face'
;;; `cperl6-look-at-leading-count': Likewise
;;; `cperl6-find-pods-heres':	localize `font-lock-variable-name-face'
...
;;; `cperl6-tips-faces':		Document REx highlighting
;;; `cperl6-praise':		Mention REx syntax highlight etc.

;;; After 5.17:
;;; `cperl6-find-sub-attrs':	Would not always manage to print error message
;;; `cperl6-find-pods-heres':	localize `font-lock-constant-face'

;;; After 5.18:
;;; `cperl6-find-pods-heres':	Misprint in REx for parsing REx
;;;				Very minor optimization
;;;				`my-cperl6-REx-modifiers-face' got quoted
;;;				Recognize "print $foo <<END" as HERE-doc.
;;;				Put `REx-interpolated' text attribute if needed
;;; `cperl6-invert-if-unless-modifiers':	New function.
;;; `cperl6-backward-to-start-of-expr': Likewise
;;; `cperl6-forward-to-end-of-expr': Likewise
;;; `cperl6-invert-if-unless':	Works in "the opposite way" too.
;;;				Cursor position on return is on the switch-word
;;;				Indents comments better
;;; `REx-interpolated':		New text attribute
;;; `cperl6-next-interpolated-REx': New function.
;;; `cperl6-next-interpolated-REx-0': Likewise.
;;; `cperl6-next-interpolated-REx-1': Likewise.
;;; "\C-c\C-x", "\C-c\C-y", "\C-c\C-v":	New keybinding for these functions
;;; Perl/Regexp menu:		3 new entries for `cperl6-next-interpolated-REx'
;;; `cperl6-praise':		Mention finded interpolated RExen.

;;; After 5.19:
;;; `cperl6-init-faces':		Highlight %$foo, @$foo too.
;;; `cperl6-short-docs':		Better docs for system, exec
;;; `cperl6-find-pods-heres':	Better detect << after print {FH} <<EOF etc.
;;;				Would not find HERE-doc ended by EOF without NL
;;; `cperl6-short-docs':		Correct not-doubled \-escapes
...
;;; Details:
;;;
;;;  New config variable `cperl6-use-v6' that switches between Perl5/Perl6
;;;  syntax.
;;;
...
;;;
;;;  Brackets/braces for arrays/hashes must
;;;  have no whitespace after variable name.
;;;
...
(if (fboundp 'eval-when-compile)
	...
	(error nil))
	  ...
	  (require 'man)
	(error nil))
      (defconst cperl6-xemacs-p (string-match "XEmacs\\|Lucid" emacs-version))
      (defvar cperl6-can-font-lock
	(or cperl6-xemacs-p
	    (and (boundp 'emacs-major-version)
		 (or window-system
		     (> emacs-major-version 20)))))
      (if cperl6-can-font-lock
	  (require 'font-lock))
      ...
      (defmacro cperl6-is-face (arg)	; Takes quoted arg
	(cond ((fboundp 'find-face)
	      ...
	      (t
	       `(boundp (,arg)))))
      (defmacro cperl6-make-face (arg descr) ; Takes unquoted arg
	(cond ((fboundp 'make-face)
	       `(make-face (quote (,arg))))
	      (t
	       `(defvar (,arg) (quote (,arg)) (,descr)))))
      (defmacro cperl6-force-face (arg descr) ; Takes unquoted arg
	`(progn
	     (or (cperl6-is-face (quote (,arg)))
		 (cperl6-make-face (,arg) (,descr)))
	     (or (boundp (quote (,arg))) ; We use unquoted variants too
		 (defvar (,arg) (quote (,arg)) (,descr)))))
      (if cperl6-xemacs-p
	  (defmacro cperl6-etags-snarf-tag (file line)
	    `(progn
		 (beginning-of-line 2)
		 (list (,file) (,line))))
	(defmacro cperl6-etags-snarf-tag (file line)
	  `(etags-snarf-tag)))
      (if cperl6-xemacs-p
	  (defmacro cperl6-etags-goto-tag-location (elt)
	    `;;(progn
...
(defun cperl6-choose-color (&rest list)
  (let (answer)
    (while list
      (or answer
...
(defgroup cperl6-indentation-details nil
  "Indentation."
  :prefix "cperl6-"
  :group 'cperl6)

(defgroup cperl6-affected-by-hairy nil
  "Variables affected by `cperl6-hairy'."
  :prefix "cperl6-"
  :group 'cperl6)

(defgroup cperl6-autoinsert-details nil
  "Auto-insert tuneup."
  :prefix "cperl6-"
  :group 'cperl6)

(defgroup cperl6-faces nil
  "Fontification colors."
  :prefix "cperl6-"
  :group 'cperl6)

(defgroup cperl6-speed nil
  "Speed vs. validity tuneup."
  :prefix "cperl6-"
  :group 'cperl6)

(defgroup cperl6-help-system nil
  "Help system tuneup."
  :prefix "cperl6-"
  :group 'cperl6)


(defcustom cperl6-extra-newline-before-brace nil
...
instead of:
  ...
  :type 'boolean
  :group 'cperl6-autoinsert-details)

(defcustom cperl6-extra-newline-before-brace-multiline
  cperl6-extra-newline-before-brace
  "*Non-nil means the same as `cperl6-extra-newline-before-brace', but
for constructs with multiline if/unless/while/until/for/foreach condition."
  :type 'boolean
  :group 'cperl6-autoinsert-details)

(defcustom cperl6-indent-level 2
  "*Indentation of Cperl6 statements with respect to containing block."
  :type 'integer
  :group 'cperl6-indentation-details)

(defcustom cperl6-lineup-step nil
  "*`cperl6-lineup' will always lineup at multiple of this number.
If nil, the value of `cperl6-indent-level' will be used."
  :type '(choice (const nil) integer)
  :group 'cperl6-indentation-details)

(defcustom cperl6-brace-imaginary-offset 0
...
(defcustom cperl6-brace-offset 0
  "*Extra indentation for braces, compared with other text in same context."
  :type 'integer
  :group 'cperl6-indentation-details)
(defcustom cperl6-label-offset -2
  "*Offset of Cperl6 label lines relative to usual indentation."
  :type 'integer
  :group 'cperl6-indentation-details)
(defcustom cperl6-min-label-indent 1
  "*Minimal offset of Cperl6 label lines."
  :type 'integer
  :group 'cperl6-indentation-details)
(defcustom cperl6-continued-statement-offset 2
  "*Extra indent for lines not starting new statements."
  :type 'integer
  :group 'cperl6-indentation-details)
(defcustom cperl6-continued-brace-offset 0
  "*Extra indent for substatements that start with open-braces.
This is in addition to cperl6-continued-statement-offset."
  :type 'integer
  :group 'cperl6-indentation-details)
(defcustom cperl6-close-paren-offset -1
  "*Extra indent for substatements that start with close-parenthesis."
  :type 'integer
  :group 'cperl6-indentation-details)

(defcustom cperl6-auto-newline nil
  "*Non-nil means automatically newline before and after braces,
and after colons and semicolons, inserted in Cperl6 code.  The following
\\[cperl6-electric-backspace] will remove the inserted whitespace.
Insertion after colons requires both this variable and
`cperl6-auto-newline-after-colon' set."
  :type 'boolean
  :group 'cperl6-autoinsert-details)

(defcustom cperl6-autoindent-on-semi nil
  "*Non-nil means automatically indent after insertion of (semi)colon.
Active if `cperl6-auto-newline' is false."
  :type 'boolean
  :group 'cperl6-autoinsert-details)

(defcustom cperl6-auto-newline-after-colon nil
  "*Non-nil means automatically newline even after colons.
Subject to `cperl6-auto-newline' setting."
  :type 'boolean
  :group 'cperl6-autoinsert-details)

(defcustom cperl6-tab-always-indent t
  "*Non-nil means TAB in Cperl6 mode should always reindent the current line,
regardless of where in the line point is when the TAB command is used."
  :type 'boolean
  :group 'cperl6-indentation-details)

(defcustom cperl6-font-lock nil
  "*Non-nil (and non-null) means Cperl6 buffers will use `font-lock-mode'.
Can be overwritten by `cperl6-hairy' if nil."
  :type '(choice (const null) boolean)
  :group 'cperl6-affected-by-hairy)

(defcustom cperl6-electric-lbrace-space nil
  "*Non-nil (and non-null) means { after $ should be preceded by ` '.
Can be overwritten by `cperl6-hairy' if nil."
  :type '(choice (const null) boolean)
  :group 'cperl6-affected-by-hairy)

(defcustom cperl6-electric-parens-string "({[]})<"
  "*String of parentheses that should be electric in Cperl6.
Closing ones are electric only if the region is highlighted."
  :type 'string
  :group 'cperl6-affected-by-hairy)

(defcustom cperl6-electric-parens nil
  "*Non-nil (and non-null) means parentheses should be electric in Cperl6.
Can be overwritten by `cperl6-hairy' if nil."
  :type '(choice (const null) boolean)
  :group 'cperl6-affected-by-hairy)
...
(defcustom cperl6-electric-parens-mark
  (and window-system
...
(defcustom cperl6-electric-linefeed nil
  "*If true, LFD should be hairy in Cperl6, otherwise C-c LFD is hairy.
In any case these two mean plain and hairy linefeeds together.
Can be overwritten by `cperl6-hairy' if nil."
  :type '(choice (const null) boolean)
  :group 'cperl6-affected-by-hairy)

(defcustom cperl6-electric-keywords nil
  "*Not-nil (and non-null) means keywords are electric in Cperl6.
Can be overwritten by `cperl6-hairy' if nil."
  :type '(choice (const null) boolean)
  :group 'cperl6-affected-by-hairy)

(defcustom cperl6-electric-backspace-untabify t
  "*Not-nil means electric-backspace will untabify in Cperl6."
  :type 'boolean
  :group 'cperl6-autoinsert-details)

(defcustom cperl6-hairy nil
  "*Not-nil means most of the bells and whistles are enabled in Cperl6.
Affects: `cperl6-font-lock', `cperl6-electric-lbrace-space',
`cperl6-electric-parens', `cperl6-electric-linefeed', `cperl6-electric-keywords',
`cperl6-info-on-command-no-prompt', `cperl6-clobber-lisp-bindings',
`cperl6-lazy-help-time'."
  :type 'boolean
  :group 'cperl6-affected-by-hairy)

(defcustom cperl6-comment-column 32
  "*Column to put comments in Cperl6 (use \\[cperl6-indent] to lineup with code)."
  :type 'integer
  :group 'cperl6-indentation-details)

(defcustom cperl6-vc-sccs-header '("($sccs) = ('%W\%' =~ /(\\d+(\\.\\d+)+)/) ;")
  "*Special version of `vc-sccs-header' that is used in Cperl6 mode buffers."
  :type '(repeat string)
  :group 'cperl6)

(defcustom cperl6-vc-rcs-header '("($rcs) = (' $Id\$ ' =~ /(\\d+(\\.\\d+)+)/);")
  "*Special version of `vc-rcs-header' that is used in Cperl6 mode buffers."
  :type '(repeat string)
     :group 'cperl6)

;; This became obsolete...
(defcustom cperl6-vc-header-alist '()
  "*What to use as `vc-header-alist' in Cperl6.
Obsolete, with newer Emacsen use `cperl6-vc-rcs-header' or
`cperl6-vc-sccs-header' instead.  If this list is empty, `vc-header-alist'
will be reconstructed basing on these two variables."
  :type '(repeat (list symbol string))
  :group 'cperl6)

(defcustom cperl6-clobber-mode-lists
  (not
   (and
...
(defcustom cperl6-info-on-command-no-prompt nil
...
(defcustom cperl6-clobber-lisp-bindings nil
  "*Not-nil (and non-null) means not overwrite C-h f.
The function is available on \\[cperl6-info-on-command], \\[cperl6-get-help].
Can be overwritten by `cperl6-hairy' if nil."
  :type '(choice (const null) boolean)
  :group 'cperl6-affected-by-hairy)

(defcustom cperl6-lazy-help-time nil
  "*Not-nil (and non-null) means to show lazy help after given idle time.
Can be overwritten by `cperl6-hairy' to be 5 sec if nil."
  :type '(choice (const null) (const nil) integer)
  :group 'cperl6-affected-by-hairy)

(defcustom cperl6-pod-face 'font-lock-comment-face
  "*The result of evaluation of this expression is used for POD highlighting."
  :type 'face
  :group 'cperl6-faces)

(defcustom cperl6-pod-head-face 'font-lock-variable-name-face
  "*The result of evaluation of this expression is used for POD highlighting.
Font for POD headers."
  :type 'face
  :group 'cperl6-faces)

(defcustom cperl6-here-face 'font-lock-string-face
...
(defvar cperl6-singly-quote-face cperl6-xemacs-p)

(defcustom cperl6-invalid-face		; Does not customize with '' on XEmacs
  (if cperl6-singly-quote-face
...
(defcustom cperl6-pod-here-fontify '(featurep 'font-lock)
  "*Not-nil after evaluation means to highlight POD and here-docs sections."
  :type 'boolean
  :group 'cperl6-faces)

(defcustom cperl6-fontify-m-as-s t
  "*Not-nil means highlight 1arg regular expressions operators same as 2arg."
  :type 'boolean
  :group 'cperl6-faces)

(defcustom cperl6-highlight-variables-indiscriminately nil
...
entered Cperl6 mode the first time will have no effect."
  :type 'boolean
  :group 'cperl6)

(defcustom cperl6-pod-here-scan t
  "*Not-nil means look for POD and here-docs sections during startup.
You can always make lookup from menu or using \\[cperl6-find-pods-heres]."
  :type 'boolean
  :group 'cperl6-speed)

(defcustom cperl6-regexp-scan t
  "*Not-nil means make marking of regular expression more thorough.
Effective only with `cperl6-pod-here-scan'."
  :type 'boolean
  :group 'cperl6-speed)

(defcustom cperl6-hook-after-change t
...
(defcustom cperl6-imenu-addback nil
  "*Not-nil means add backreferences to generated `imenu's.
May require patched `imenu' and `imenu-go'.  Obsolete."
  :type 'boolean
  :group 'cperl6-help-system)

(defcustom cperl6-max-help-size 66
  "*Non-nil means shrink-wrapping of info-buffer allowed up to these percents."
  :type '(choice integer (const nil))
  :group 'cperl6-help-system)

(defcustom cperl6-shrink-wrap-info-frame t
  "*Non-nil means shrink-wrapping of info-buffer-frame allowed."
  :type 'boolean
  :group 'cperl6-help-system)

(defcustom cperl6-info-page "perl"
  "*Name of the info page containing perl docs.
Older version of this page was called `perl5', newer `perl'."
  :type 'string
  :group 'cperl6-help-system)

(defcustom cperl6-use-syntax-table-text-property
  (boundp 'parse-sexp-lookup-properties)
  "*Non-nil means Cperl6 sets up and uses `syntax-table' text property."
  :type 'boolean
  :group 'cperl6-speed)

(defcustom cperl6-use-syntax-table-text-property-for-tags
  cperl6-use-syntax-table-text-property
  "*Non-nil means: set up and use `syntax-table' text property generating TAGS."
  :type 'boolean
  :group 'cperl6-speed)

(defcustom cperl6-scan-files-regexp "\\.\\([pP][Llm6]\\|xs\\)$"
  "*Regexp to match files to scan when generating TAGS."
  :type 'regexp
  :group 'cperl6)

(defcustom cperl6-noscan-files-regexp
  "/\\(\\.\\.?\\|SCCS\\|RCS\\|CVS\\|blib\\)$"
  "*Regexp to match files/dirs to skip when generating TAGS."
  :type 'regexp
  :group 'cperl6)

(defcustom cperl6-regexp-indent-step nil
  "*Indentation used when beautifying regexps.
If nil, the value of `cperl6-indent-level' will be used."
  :type '(choice integer (const nil))
  :group 'cperl6-indentation-details)

(defcustom cperl6-indent-left-aligned-comments t
  "*Non-nil means that the comment starting in leftmost column should indent."
  :type 'boolean
  :group 'cperl6-indentation-details)

(defcustom cperl6-under-as-char t
  "*Non-nil means that the _ (underline) should be treated as word char."
  :type 'boolean
  :group 'cperl6)

(defcustom cperl6-extra-perl-args ""
  "*Extra arguments to use when starting Perl.
Currently used with `cperl6-check-syntax' only."
  :type 'string
  :group 'cperl6)

(defcustom cperl6-message-electric-keyword t
  "*Non-nil means that the `cperl6-electric-keyword' prints a help message."
  :type 'boolean
  :group 'cperl6-help-system)

(defcustom cperl6-indent-region-fix-constructs 1
  "*Amount of space to insert between `}' and `else' or `elsif'
in `cperl6-indent-region'.  Set to nil to leave as is.  Values other
than 1 and nil will probably not work."
  :type '(choice (const nil) (const 1))
  :group 'cperl6-indentation-details)

(defcustom cperl6-break-one-line-blocks-when-indent t
...
(defcustom cperl6-fix-hanging-brace-when-indent t
...
(defcustom cperl6-merge-trailing-else t
  "*Non-nil means that BLOCK-end `}' followed by else/elsif/continue
may be merged to be on the same line when indenting a region."
  :type 'boolean
  :group 'cperl6-indentation-details)

(defcustom cperl6-indent-parens-as-block nil
  "*Non-nil means that non-block ()-, {}- and []-groups are indented as blocks,
but for trailing \",\" inside the group, which won't increase indentation.
One should tune up `cperl6-close-paren-offset' as well."
  :type 'boolean
  :group 'cperl6-indentation-details)

(defcustom cperl6-syntaxify-by-font-lock
  (and cperl6-can-font-lock
       (boundp 'parse-sexp-lookup-properties))
  "*Non-nil means that Cperl6 uses `font-lock's routines for syntaxification."
  :type '(choice (const message) boolean)
  :group 'cperl6-speed)

(defcustom cperl6-syntaxify-unwind
  t
  "*Non-nil means that Cperl6 unwinds to a start of a long construction
when syntaxifying a chunk of buffer."
  :type 'boolean
  :group 'cperl6-speed)

(defcustom cperl6-syntaxify-for-menu
  t
  "*Non-nil means that Cperl6 syntaxifies up to the point before showing menu.
This way enabling/disabling of menu items is more correct."
  :type 'boolean
  :group 'cperl6-speed)

(defcustom cperl6-ps-print-face-properties
  '((font-lock-keyword-face		nil nil		bold shadow)
    ...
    (cperl6-array-face			nil "LightGray"	bold underline)
    (cperl6-hash-face			nil "LightGray"	bold italic underline)
    (font-lock-comment-face		nil "LightGray"	italic)
    (font-lock-string-face		nil nil		italic underline)
    (cperl6-nonoverridable-face		nil nil		italic underline)
  ...
  "List given as an argument to `ps-extend-face-list' in `cperl6-ps-print'."
...
(defcustom cperl6-use-v6 nil
  "*Non-nil means that Cperl6 parses for Perl6 syntax."
  :type 'boolean
  :group 'cperl6)

(defun cperl6-looks-like-perl6 ()
  ;; Searches for typical Perl6 patterns and returns true if found.
  ;; TODO: Tooooo lame.
  (or
...
(if cperl6-can-font-lock
    (progn
      (defvar cperl6-dark-background
	(cperl6-choose-color "navy" "os2blue" "darkgreen"))
      (defvar cperl6-dark-foreground
	(cperl6-choose-color "orchid1" "orange"))

      (defface cperl6-nonoverridable-face
	(` ((((class grayscale) (background light))
	    ...
	    (((class color) (background dark))
	     (:foreground (, cperl6-dark-foreground)))
	    (t (:bold t :underline t))))
	"Font Lock mode face used to highlight array names."
	:group 'cperl6-faces)

      (defface cperl6-array-face
	(` ((((class grayscale) (background light))
	    ...
	    (((class color) (background dark))
	     (:foreground "yellow" :background (, cperl6-dark-background) :bold t))
	    (t (:bold t))))
	"Font Lock mode face used to highlight array names."
	:group 'cperl6-faces)

      (defface cperl6-hash-face
	(` ((((class grayscale) (background light))
	    ...
	    (((class color) (background dark))
	     (:foreground "Red" :background (, cperl6-dark-background) :bold t :italic t))
	    (t (:bold t :italic t))))
...
(defvar cperl6-tips 'please-ignore-this-line
  "Get maybe newer version of this package from
  http://ilyaz.org/software/emacs
Subdirectory `cperl6-mode' may contain yet newer development releases and/or
patches to related files.

For best results apply to an older Emacs the patches from
  ftp://ftp.math.ohio-state.edu/pub/users/ilya/cperl6-mode/patches
...
If your Emacs does not default to `cperl6-mode' on Perl files, and you
...
or
  ...
  (defalias 'perl-mode 'cperl6-mode)
...
Though with contemporary Emaxen Cperl6 mode should maintain the correct
...
In cases of more severe confusion sometimes it is helpful to do
  ...
  M-x load-l RET cperl6-mode RET
  M-x norm RET

or

  \\[load-library] cperl6-mode RET
  \\[normal-mode]

Before reporting (non-)problems look in the problem section of online
micro-docs on what I know about Cperl6 problems.")

(defvar cperl6-problems 'please-ignore-this-line
  "Description of problems in Cperl6 mode.
...
paragraph.  It also triggers a bug in some versions of Emacs (Cperl6 tries
to detect it and bulk out).

See documentation of a variable `cperl6-problems-old-emaxen' for the
problems which disappear if you upgrade Emacs to a reasonably new
version (20.3 for RMS Emacs, and those of 2004 for XEmacs).")

(defvar cperl6-problems-old-emaxen 'please-ignore-this-line
  "Description of problems in Cperl6 mode specific for older Emacs versions.
...
Even with older Emacsen Cperl6 mode tries to corrects some Emacs
...
properties.  Patches are available on the main Cperl6 download site,
and on CPAN.
...
via `cperl6-use-syntax-table-text-property'.")

(defvar cperl6-non-problems 'please-ignore-this-line
"As you know from `problems' section, Perl syntax is too hard for Cperl6 on
...
Try to help Cperl6: add comments with embedded quotes to fix Cperl6
misunderstandings about the end of quotation:

$a='500$';      # ';

You won't need it too often.  The reason: $ \"quotes\" the following
character (this saves a life a lot of times in Cperl6), thus due to
...
(defvar cperl6-praise 'please-ignore-this-line
  "Advantages of Cperl6 mode.
...
(defvar cperl6-speed 'please-ignore-this-line
  "This is an incomplete compendium of what is available in other parts
of Cperl6 documentation.  (Please inform me if I skept anything.)

There is a perception that Cperl6 is slower than alternatives.  This part
of documentation is designed to overcome this misconception.

*By default* Cperl6 tries to enable the most comfortable settings.
From most points of view, correctly working package is infinitely more
comfortable than a non-correctly working one, thus by default Cperl6
prefers correctness over speed.  Below is the guide how to change
settings if your preferences are different.

A)  Speed of loading the file.  When loading file, Cperl6 may perform a
...
syntax-parsing routines, and marks them up so that either
    ...
    A1) Cperl6 may work around these deficiencies (for big chunks, mostly
        PODs and HERE-documents), or
    A2) On capable Emaxen Cperl6 will use improved syntax-handlings
...
B) Speed of editing operations.
    ...
    constructs be wrongly recognized by Cperl6, thus may lead to
    wrongly matched parentheses, wrong indentation, etc.

    One can unset `cperl6-syntaxify-unwind'.  This might speed up editing
    of, say, long POD sections.")

(defvar cperl6-tips-faces 'please-ignore-this-line
  "Cperl6 mode uses following faces for highlighting:

  `cperl6-array-face'		Array names
  `cperl6-hash-face'		Hash names
  ...
  `cperl6-nonoverridable-face'	Non-overridable keywords, modifiers of RExen
  ...
  `cperl6-invalid-face'		Trailing whitespace
...
In regular expressions (except character classes):
  ...
  `cperl6-nonoverridable-face'	Modifiers, as gism in m/REx/gism
...
(defmacro cperl6-define-key (emacs-key definition &optional xemacs-key)
  (` (define-key cperl6-mode-map
       (, (if xemacs-key
	      (` (if cperl6-xemacs-p (, xemacs-key) (, emacs-key)))
	    emacs-key))
       (, definition))))

(defvar cperl6-del-back-ch
...
(and (vectorp cperl6-del-back-ch) (= (length cperl6-del-back-ch) 1)
     (setq cperl6-del-back-ch (aref cperl6-del-back-ch 0)))

(defun cperl6-mark-active () (mark))	; Avoid undefined warning
(if cperl6-xemacs-p
    (progn
      ;; "Active regions" are on: use region only if active
      ;; "Active regions" are off: use region unconditionally
      (defun cperl6-use-region-p ()
	(if zmacs-regions (mark) t)))
  (defun cperl6-use-region-p ()
    (if transient-mark-mode mark-active t))
  (defun cperl6-mark-active () mark-active))

(defsubst cperl6-enable-font-lock ()
  cperl6-can-font-lock)

(defun cperl6-putback-char (c)		; Emacs 19
  (set 'unread-command-events (list c))) ; Avoid undefined warning

(if (boundp 'unread-command-events)
    (if cperl6-xemacs-p
	(defun cperl6-putback-char (c)	; XEmacs >= 19.12
	  (setq unread-command-events (list (eval '(character-to-event c))))))
  (defun cperl6-putback-char (c)		; XEmacs <= 19.11
...
(defvar cperl6-do-not-fontify
  (if (string< emacs-version "19.30")
      'fontified
    'lazy-lock)
  "Text property which inhibits refontification.")

(defsubst cperl6-put-do-not-fontify (from to &optional post)
  ;; If POST, do not do it with postponed fontification
  (if (and post cperl6-syntaxify-by-font-lock)
      nil
    (put-text-property (max (point-min) (1- from))
		       to cperl6-do-not-fontify t)))

(defcustom cperl6-mode-hook nil
  "Hook run by Cperl6 mode."
  :type 'hook
  :group 'cperl6)

(defvar cperl6-syntax-state nil)
(defvar cperl6-syntax-done-to nil)
(defvar cperl6-emacs-can-parse (> (length (save-excursion
					   (parse-partial-sexp (point) (point)))) 9))

;; Make customization possible "in reverse"
(defsubst cperl6-val (symbol &optional default hairy)
  (cond
   ((eq (symbol-value symbol) 'null) default)
   (cperl6-hairy (or hairy t))
   (t (symbol-value symbol))))


(defun cperl6-make-indent (column &optional minimum keep)
...
(if (fboundp 'eval-when-compile)
    (eval-when-compile
	  ...
	  (defmacro cperl6-ps-extend-face-list (arg)
	    (` (ps-extend-face-list (, arg))))
	(defmacro cperl6-ps-extend-face-list (arg)
	  (` (error "This version of Emacs has no `ps-extend-face-list'"))))
      ;; Calling `cperl6-enable-font-lock' below doesn't compile on XEmacs,
...
(defvar cperl6-mode-abbrev-table nil
  "Abbrev table in use in Cperl6 mode buffers.")

(add-hook 'edit-var-mode-alist '(perl-mode (regexp . "^cperl6-")))

(defvar cperl6-mode-map () "Keymap used in Cperl6 mode.")

(if cperl6-mode-map nil
  (setq cperl6-mode-map (make-sparse-keymap))
  (cperl6-define-key "{" 'cperl6-electric-lbrace)
  (cperl6-define-key "[" 'cperl6-electric-paren)
  (cperl6-define-key "(" 'cperl6-electric-paren)
  (cperl6-define-key "<" 'cperl6-electric-paren)
  (cperl6-define-key "}" 'cperl6-electric-brace)
  (cperl6-define-key "]" 'cperl6-electric-rparen)
  (cperl6-define-key ")" 'cperl6-electric-rparen)
  (cperl6-define-key ";" 'cperl6-electric-semi)
  (cperl6-define-key ":" 'cperl6-electric-terminator)
  (cperl6-define-key "\C-j" 'newline-and-indent)
  (cperl6-define-key "\C-c\C-j" 'cperl6-linefeed)
  (cperl6-define-key "\C-c\C-t" 'cperl6-invert-if-unless)
  (cperl6-define-key "\C-c\C-a" 'cperl6-toggle-auto-newline)
  (cperl6-define-key "\C-c\C-k" 'cperl6-toggle-abbrev)
  (cperl6-define-key "\C-c\C-w" 'cperl6-toggle-construct-fix)
  (cperl6-define-key "\C-c\C-f" 'auto-fill-mode)
  (cperl6-define-key "\C-c\C-e" 'cperl6-toggle-electric)
  (cperl6-define-key "\C-c\C-b" 'cperl6-find-bad-style)
  (cperl6-define-key "\C-c\C-p" 'cperl6-pod-spell)
  (cperl6-define-key "\C-c\C-d" 'cperl6-here-doc-spell)
  (cperl6-define-key "\C-c\C-n" 'cperl6-narrow-to-here-doc)
  (cperl6-define-key "\C-c\C-v" 'cperl6-next-interpolated-REx)
  (cperl6-define-key "\C-c\C-x" 'cperl6-next-interpolated-REx-0)
  (cperl6-define-key "\C-c\C-y" 'cperl6-next-interpolated-REx-1)
  (cperl6-define-key "\C-c\C-ha" 'cperl6-toggle-autohelp)
  (cperl6-define-key "\C-c\C-hp" 'cperl6-perldoc)
  (cperl6-define-key "\C-c\C-hP" 'cperl6-perldoc-at-point)
  (cperl6-define-key "\e\C-q" 'cperl6-indent-exp) ; Usually not bound
  (cperl6-define-key [?\C-\M-\|] 'cperl6-lineup
		    [(control meta |)])
  ;;(cperl6-define-key "\M-q" 'cperl6-fill-paragraph)
  ;;(cperl6-define-key "\e;" 'cperl6-indent-for-comment)
  (cperl6-define-key "\177" 'cperl6-electric-backspace)
  (cperl6-define-key "\t" 'cperl6-indent-command)
  ;; don't clobber the backspace binding:
  (cperl6-define-key "\C-c\C-hF" 'cperl6-info-on-command
		    [(control c) (control h) F])
  (if (cperl6-val 'cperl6-clobber-lisp-bindings)
      (progn
	(cperl6-define-key "\C-hf"
			  ;;(concat (char-to-string help-char) "f") ; does not work
			  'cperl6-info-on-command
			  [(control h) f])
	(cperl6-define-key "\C-hv"
			  ;;(concat (char-to-string help-char) "v") ; does not work
			  'cperl6-get-help
			  [(control h) v])
	(cperl6-define-key "\C-c\C-hf"
			  ;;(concat (char-to-string help-char) "f") ; does not work
			  (key-binding "\C-hf")
			  [(control c) (control h) f])
	(cperl6-define-key "\C-c\C-hv"
			  ;;(concat (char-to-string help-char) "v") ; does not work
			  (key-binding "\C-hv")
			  [(control c) (control h) v]))
    (cperl6-define-key "\C-c\C-hf" 'cperl6-info-on-current-command
		      [(control c) (control h) f])
    (cperl6-define-key "\C-c\C-hv"
		      ;;(concat (char-to-string help-char) "v") ; does not work
		      'cperl6-get-help
		      [(control c) (control h) v]))
  (if (and cperl6-xemacs-p
	   (<= emacs-minor-version 11) (<= emacs-major-version 19))
      (progn
	;; substitute-key-definition is usefulness-deenhanced...
	;;;;;(cperl6-define-key "\M-q" 'cperl6-fill-paragraph)
	(cperl6-define-key "\e;" 'cperl6-indent-for-comment)
	(cperl6-define-key "\e\C-\\" 'cperl6-indent-region))
    (or (boundp 'fill-paragraph-function)
	(substitute-key-definition
	 'fill-paragraph 'cperl6-fill-paragraph
	 cperl6-mode-map global-map))
    (substitute-key-definition
     'indent-sexp 'cperl6-indent-exp
     cperl6-mode-map global-map)
    (substitute-key-definition
     'indent-region 'cperl6-indent-region
     cperl6-mode-map global-map)
    (substitute-key-definition
     'indent-for-comment 'cperl6-indent-for-comment
     cperl6-mode-map global-map)))

(defvar cperl6-menu)
(defvar cperl6-lazy-installed)
(defvar cperl6-old-style nil)
(condition-case nil
    (progn
      (require 'easymenu)
      (easy-menu-define
       cperl6-menu cperl6-mode-map "Menu for Cperl6 mode"
       '("Perl"
	 ["Beginning of function" beginning-of-defun t]
	 ["End of function" end-of-defun t]
	 ["Mark function" mark-defun t]
	 ["Indent expression" cperl6-indent-exp t]
	 ["Fill paragraph/comment" cperl6-fill-paragraph t]
	 "----"
	 ["Line up a construction" cperl6-lineup (cperl6-use-region-p)]
	 ["Invert if/unless/while etc" cperl6-invert-if-unless t]
	 ("Regexp"
	  ["Beautify" cperl6-beautify-regexp
	   cperl6-use-syntax-table-text-property]
	  ["Beautify one level deep" (cperl6-beautify-regexp 1)
	   cperl6-use-syntax-table-text-property]
	  ["Beautify a group" cperl6-beautify-level
	   cperl6-use-syntax-table-text-property]
	  ["Beautify a group one level deep" (cperl6-beautify-level 1)
	   cperl6-use-syntax-table-text-property]
	  ["Contract a group" cperl6-contract-level
	   cperl6-use-syntax-table-text-property]
	  ["Contract groups" cperl6-contract-levels
	   cperl6-use-syntax-table-text-property]
	  "----"
	  ["Find next interpolated" cperl6-next-interpolated-REx
	   (next-single-property-change (point-min) 'REx-interpolated)]
	  ["Find next interpolated (no //o)"
	   cperl6-next-interpolated-REx-0
	  ...
	  ["Find next interpolated (neither //o nor whole-REx)"
	   cperl6-next-interpolated-REx-1
	   (text-property-any (point-min) (point-max) 'REx-interpolated t)])
	 ["Insert spaces if needed to fix style" cperl6-find-bad-style t]
	 ["Refresh \"hard\" constructions" cperl6-find-pods-heres t]
	 "----"
	 ["Indent region" cperl6-indent-region (cperl6-use-region-p)]
	 ["Comment region" cperl6-comment-region (cperl6-use-region-p)]
	 ["Uncomment region" cperl6-uncomment-region (cperl6-use-region-p)]
	 "----"
	 ...
	 ["Check syntax" cperl6-check-syntax (fboundp 'mode-compile)]
	 "----"
	 ["Debugger" cperl6-db t]
	 "----"
	 ("Tools"
	  ["Imenu" imenu (fboundp 'imenu)]
	  ["Imenu on Perl Info" cperl6-imenu-on-info (featurep 'imenu)]
	  "----"
	  ["Ispell PODs" cperl6-pod-spell
	   ...
	   (or
	    ...
	    (< (progn
		 (and cperl6-syntaxify-for-menu
		      (cperl6-update-syntaxification (point-max) (point-max)))
		 (next-single-property-change (point-min) 'in-pod nil (point-max)))
	       (point-max)))]
	  ["Ispell HERE-DOCs" cperl6-here-doc-spell
	   (< (progn
		(and cperl6-syntaxify-for-menu
		     (cperl6-update-syntaxification (point-max) (point-max)))
		(next-single-property-change (point-min) 'here-doc-group nil (point-max)))
	      (point-max))]
	  ["Narrow to this HERE-DOC" cperl6-narrow-to-here-doc
	   (eq 'here-doc  (progn
		(and cperl6-syntaxify-for-menu
		     (cperl6-update-syntaxification (point) (point)))
		(get-text-property (point) 'syntax-type)))]
	  ["Select this HERE-DOC or POD section"
	   cperl6-select-this-pod-or-here-doc
	   (memq (progn
		   (and cperl6-syntaxify-for-menu
			(cperl6-update-syntaxification (point) (point)))
		   (get-text-property (point) 'syntax-type))
		 '(here-doc pod))]
	  "----"
	  ["Cperl6 pretty print (exprmntl)" cperl6-ps-print
	   (fboundp 'ps-extend-face-list)]
	  "----"
	  ["Syntaxify region" cperl6-find-pods-heres-region
	   (cperl6-use-region-p)]
	  ["Profile syntaxification" cperl6-time-fontification t]
	  ["Debug errors in delayed fontification" cperl6-emulate-lazy-lock t]
	  ["Debug unwind for syntactic scan" cperl6-toggle-set-debug-unwind t]
	  ["Debug backtrace on syntactic scan (BEWARE!!!)"
	   (cperl6-toggle-set-debug-unwind nil t) t]
	  "----"
	  ["Class Hierarchy from TAGS" cperl6-tags-hier-init t]
	  ;;["Update classes" (cperl6-tags-hier-init t) tags-table-list]
	  ("Tags"
;;;	     ["Create tags for current file" cperl6-etags t]
;;;	     ["Add tags for current file" (cperl6-etags t) t]
;;;	     ["Create tags for Perl files in directory" (cperl6-etags nil t) t]
;;;	     ["Add tags for Perl files in directory" (cperl6-etags t t) t]
;;;	     ["Create tags for Perl files in (sub)directories"
;;;	      (cperl6-etags nil 'recursive) t]
;;;	     ["Add tags for Perl files in (sub)directories"
;;;	      (cperl6-etags t 'recursive) t])
;;;; cperl6-write-tags (&optional file erase recurse dir inbuffer)
	   ["Create tags for current file" (cperl6-write-tags nil t) t]
	   ["Add tags for current file" (cperl6-write-tags) t]
	   ["Create tags for Perl files in directory"
	    (cperl6-write-tags nil t nil t) t]
	   ["Add tags for Perl files in directory"
	    (cperl6-write-tags nil nil nil t) t]
	   ["Create tags for Perl files in (sub)directories"
	    (cperl6-write-tags nil t t t) t]
	   ["Add tags for Perl files in (sub)directories"
	    (cperl6-write-tags nil nil t t) t]))
	 ("Perl docs"
	  ["Define word at point" imenu-go-find-at-position
	   (fboundp 'imenu-go-find-at-position)]
	  ["Help on function" cperl6-info-on-command t]
	  ["Help on function at point" cperl6-info-on-current-command t]
	  ["Help on symbol at point" cperl6-get-help t]
	  ["Perldoc" cperl6-perldoc t]
	  ["Perldoc on word at point" cperl6-perldoc-at-point t]
	  ["View manpage of POD in this file" cperl6-build-manpage t]
	  ["Auto-help on" cperl6-lazy-install
	   (and (fboundp 'run-with-idle-timer)
		(not cperl6-lazy-installed))]
	  ["Auto-help off" cperl6-lazy-unstall
	   (and (fboundp 'run-with-idle-timer)
		cperl6-lazy-installed)])
	 ("Toggle..."
	  ["Auto newline" cperl6-toggle-auto-newline t]
	  ["Electric parens" cperl6-toggle-electric t]
	  ["Electric keywords" cperl6-toggle-abbrev t]
	  ["Fix whitespace on indent" cperl6-toggle-construct-fix t]
	  ["Auto-help on Perl constructs" cperl6-toggle-autohelp t]
	  ["Auto fill" auto-fill-mode t])
	 ("Indent styles..."
	  ["Cperl6" (cperl6-set-style "Cperl6") t]
	  ...
	  ["BSD" (cperl6-set-style "BSD") t]
	  ["Whitesmith" (cperl6-set-style "Whitesmith") t]
	  ["Current" (cperl6-set-style "Current") t]
	  ["Memorized" (cperl6-set-style-back) cperl6-old-style])
	 ("Micro-docs"
	  ["Tips" (describe-variable 'cperl6-tips) t]
	  ["Problems" (describe-variable 'cperl6-problems) t]
	  ["Non-problems" (describe-variable 'cperl6-non-problems) t]
	  ["Speed" (describe-variable 'cperl6-speed) t]
	  ["Praise" (describe-variable 'cperl6-praise) t]
	  ["Faces" (describe-variable 'cperl6-tips-faces) t]
	  ["Cperl6 mode" (describe-function 'cperl6-mode) t]
	  ["Cperl6 version"
	   (message "The version of master-file for this Cperl6 is %s"
		    cperl6-version) t]))))
  (error nil))
...
(defconst cperl6-maybe-white-and-comment-rex "[ \t\n]*\\(#[^\n]*\n[ \t\n]*\\)*"
...
(defconst cperl6-white-and-comment-rex "\\([ \t\n]\\|#[^\n]*\n\\)+"
"Regular expression to match whitespace with interpspersed comments.
Should contain exactly one group.")


;;; Is incorporated in `cperl6-imenu--function-name-regexp-perl'
;;; `cperl6-outline-regexp', `defun-prompt-regexp'.
...
(defsubst cperl6-after-sub-regexp (named attr) ; 9 groups without attr...
...
the last)."
  (concat				; Assume n groups before this...
   "\\("				; n+1=name-group
     cperl6-white-and-comment-rex	; n+2=pre-name
     "\\(::[a-zA-Z_0-9:']+\\|[a-zA-Z_'][a-zA-Z_0-9:']*\\)" ; n+3=name
   "\\)"				; END n+1=name-group
   (if named "" "?")
   "\\("				; n+4=proto-group
     cperl6-maybe-white-and-comment-rex	; n+5=pre-proto
     "\\(([^()]*)\\)"			; n+6=prototype
   "\\)?"				; END n+4=proto-group
   "\\("				; n+7=attr-group
     cperl6-maybe-white-and-comment-rex	; n+8=pre-attr
     "\\("				; n+9=start-attr
        ":"
	(if attr (concat
		  "\\("
		     cperl6-maybe-white-and-comment-rex ; whitespace-comments
		     "\\(\\sw\\|_\\)+"	; attr-name
...
;;; Details of groups in this are used in `cperl6-imenu--create-perl-index'
;;;  and `cperl6-outline-level'.
  ...
  (concat
   "^\\("				; 1 = all
       "\\([ \t]*package"		; 2 = package-group
	    ...
	    cperl6-white-and-comment-rex ; 4 = pre-package-name
	       "\\([a-zA-Z_0-9:']+\\)\\)?\\)" ; 5 = package-name
       "\\|"
          "[ \t]*\\(\\(multi\\|proto\\)[ \t]*\\)?\\(coro\\|sub\\|method\\|submethod\\)?" ; perl6
	  (cperl6-after-sub-regexp 'named nil) ; 11=name 14=proto 17=attr-start
	  cperl6-maybe-white-and-comment-rex	; 18=pre-block
   "\\|"
     "=head\\([1-4]\\)[ \t]+"		; 19=level
     "\\([^\n]+\\)$"			; 20=text
   "\\)"))
...
(defvar cperl6-outline-regexp
  (concat cperl6-imenu--function-name-regexp-perl "\\|" "\\`"))

(defvar cperl6-mode-syntax-table nil
  "Syntax table in use in Cperl6 mode buffers.")

(defvar cperl6-string-syntax-table nil
  "Syntax table in use in Cperl6 mode string-like chunks.")
...
(if cperl6-mode-syntax-table
    ()
  (setq cperl6-mode-syntax-table (make-syntax-table))
  (modify-syntax-entry ?\\ "\\" cperl6-mode-syntax-table)
  (modify-syntax-entry ?/ "." cperl6-mode-syntax-table)
  (modify-syntax-entry ?* "." cperl6-mode-syntax-table)
  (modify-syntax-entry ?+ "." cperl6-mode-syntax-table)
  (modify-syntax-entry ?- "." cperl6-mode-syntax-table)
  ...
  (modify-syntax-entry ?& "." cperl6-mode-syntax-table)
  (modify-syntax-entry ?$ "\\" cperl6-mode-syntax-table)
  (modify-syntax-entry ?\n ">" cperl6-mode-syntax-table)
  (modify-syntax-entry ?# "<" cperl6-mode-syntax-table)
  (modify-syntax-entry ?' "\"" cperl6-mode-syntax-table)
  (modify-syntax-entry ?` "\"" cperl6-mode-syntax-table)
  (if cperl6-under-as-char
      (modify-syntax-entry ?_ "w" cperl6-mode-syntax-table))
  (modify-syntax-entry ?: "_" cperl6-mode-syntax-table)
  (modify-syntax-entry ?| "." cperl6-mode-syntax-table)
  (setq cperl6-string-syntax-table (copy-syntax-table cperl6-mode-syntax-table))
  (modify-syntax-entry ?$ "." cperl6-string-syntax-table)
  (modify-syntax-entry ?\{ "." cperl6-string-syntax-table)
  (modify-syntax-entry ?\} "." cperl6-string-syntax-table)
  (modify-syntax-entry ?# "." cperl6-string-syntax-table)) ; (?# comment )
...
;;;###autoload
(fset 'perl-mode 'cperl6-mode)
(defvar cperl6-faces-init nil)
;; Fix for msb.el
(defvar cperl6-msb-fixed nil)
...
(if (fboundp 'defvaralias)
    (let ((f 'defvaralias)) ; Some functions deduce stuff from the mode name...
      (funcall f 'cperl6-font-lock-keywords   'perl-font-lock-keywords)
      (funcall f 'cperl6-font-lock-keywords-1 'perl-font-lock-keywords-1)
      (funcall f 'cperl6-font-lock-keywords-2 'perl-font-lock-keywords-2)))

(defvar cperl6-use-major-mode 'perl-mode)
(defvar cperl6-font-lock-multiline-start nil)
(defvar cperl6-font-lock-multiline nil)
(defvar cperl6-compilation-error-regexp-alist nil)
(defvar cperl6-font-locking nil)
...
since most the time you mean \"less\".  Cperl6 mode tries to guess
whether you want to type pair <>, and inserts is if it
appropriate.  You can set `cperl6-electric-parens-string' to the string that
contains the parenths from the above list you want to be electrical.
Electricity of parenths is controlled by `cperl6-electric-parens'.
You may also set `cperl6-electric-parens-mark' to have electric parens
look for active mark and \"embrace\" a region if possible.'

Cperl6 mode provides expansion of the Perl control constructs:

   if, else, elsif, unless, while, until, continue, do,
   for, foreach, formy and foreachmy.

and POD directives (Disabled by default, see `cperl6-electric-keywords'.)
...
typing \\[cperl6-linefeed] places you - appropriately indented - on a
new line between the braces (if you typed \\[cperl6-linefeed] in a POD
directive line, then appropriate number of new lines is inserted).

If Cperl6 decides that you want to insert \"English\" style construct like

            bite if angry;

it will not do any expansion.  See also help on variable
`cperl6-extra-newline-before-brace'.  (Note that one can switch the
help message on expansion by setting `cperl6-message-electric-keyword'
to nil.)

\\[cperl6-linefeed] is a convenience replacement for typing carriage
...
see documentation on `cperl6-electric-linefeed'.

Use \\[cperl6-invert-if-unless] to change a construction of the form
...
Setting the variable `cperl6-font-lock' to t switches on font-lock-mode
\(even with older Emacsen), `cperl6-electric-lbrace-space' to t switches
on electric space between $ and {, `cperl6-electric-parens-string' is
the string that contains parentheses that should be electric in Cperl6
\(see also `cperl6-electric-parens-mark' and `cperl6-electric-parens'),
setting `cperl6-electric-keywords' enables electric expansion of
control structures in Cperl6.  `cperl6-electric-linefeed' governs which
one of two linefeed behavior is preferable.  You can enable all these
options simultaneously (recommended mode of use) by setting
`cperl6-hairy' to t.  In this case you can switch separate options off
by setting them to `null'.  Note that one may undo the extra
whitespace inserted by semis and braces in `auto-newline'-mode by
consequent \\[cperl6-electric-backspace].

If your site has perl5 documentation in info format, you can use commands
\\[cperl6-info-on-current-command] and \\[cperl6-info-on-command] to access it.
These keys run commands `cperl6-info-on-current-command' and
`cperl6-info-on-command', which one is which is controlled by variable
`cperl6-info-on-command-no-prompt' and `cperl6-clobber-lisp-bindings'
\(in turn affected by `cperl6-hairy').

Even if you have no info-format documentation, short one-liner-style
help is available on \\[cperl6-get-help], and one can run perldoc or
man via menu.

It is possible to show this help automatically after some idle time.
This is regulated by variable `cperl6-lazy-help-time'.  Default with
`cperl6-hairy' (if the value of `cperl6-lazy-help-time' is nil) is 5
secs idle time .  It is also possible to switch this on/off from the
menu, or via \\[cperl6-toggle-autohelp].  Requires `run-with-idle-timer'.

Use \\[cperl6-lineup] to vertically lineup some construction - put the
beginning of the region at the start of construction, and make region
span the needed amount of lines.

Variables `cperl6-pod-here-scan', `cperl6-pod-here-fontify',
`cperl6-pod-face', `cperl6-pod-head-face' control processing of POD and
here-docs sections.  With capable Emaxen results of scan are used
for indentation too, otherwise they are used for highlighting only.

Variables controlling indentation style:
 `cperl6-tab-always-indent'
    Non-nil means TAB in Cperl6 mode should always reindent the current line,
    regardless of where in the line point is when the TAB command is used.
 `cperl6-indent-left-aligned-comments'
    Non-nil means that the comment starting in leftmost column should indent.
 `cperl6-auto-newline'
    Non-nil means automatically newline before and after braces,
    ...
    Insertion after colons requires both this variable and
    `cperl6-auto-newline-after-colon' set.
 `cperl6-auto-newline-after-colon'
    Non-nil means automatically newline even after colons.
    Subject to `cperl6-auto-newline' setting.
 `cperl6-indent-level'
 ...
 `cperl6-continued-statement-offset'
    Extra indentation given to a substatement, such as the
    then-clause of an if, or body of a while, or just a statement continuation.
 `cperl6-continued-brace-offset'
    Extra indentation given to a brace that starts a substatement.
    This is in addition to `cperl6-continued-statement-offset'.
 `cperl6-brace-offset'
    Extra indentation for line if it starts with an open brace.
 `cperl6-brace-imaginary-offset'
    An open brace following other text is treated as if it the line started
    this far to the right of the actual line indentation.
 `cperl6-label-offset'
    Extra indentation for line that is a label.
 `cperl6-min-label-indent'
    Minimal indentation for line that is a label.

Settings for K&R and BSD indentation styles are
  `cperl6-indent-level'                5    8
  `cperl6-continued-statement-offset'  5    8
  `cperl6-brace-offset'               -5   -8
  `cperl6-label-offset'               -5   -8

Cperl6 knows several indentation styles, and may bulk set the
corresponding variables.  Use \\[cperl6-set-style] to do this.  Use
\\[cperl6-set-style-back] to restore the memorized preexisting values
\(both available from menu).

If `cperl6-indent-level' is 0, the statement after opening brace in
column 0 is indented on
`cperl6-brace-offset'+`cperl6-continued-statement-offset'.

Turning on Cperl6 mode calls the hooks in the variable `cperl6-mode-hook'
with no args.
...
`cperl6-non-problems', `cperl6-praise', `cperl6-speed'."
  ...
  (use-local-map cperl6-mode-map)
  (if (cperl6-val 'cperl6-electric-linefeed)
      (progn
	(local-set-key "\C-J" 'cperl6-linefeed)
	(local-set-key "\C-C\C-J" 'newline-and-indent)))
  (if (and
       (cperl6-val 'cperl6-clobber-lisp-bindings)
       (cperl6-val 'cperl6-info-on-command-no-prompt))
      (progn
	;; don't clobber the backspace binding:
	(cperl6-define-key "\C-hf" 'cperl6-info-on-current-command [(control h) f])
	(cperl6-define-key "\C-c\C-hf" 'cperl6-info-on-command
			  [(control c) (control h) f])))
  (setq major-mode cperl6-use-major-mode)
  (setq mode-name "Cperl6")
  (if (not cperl6-mode-abbrev-table)
      (let ((prev-a-c abbrevs-changed))
	(define-abbrev-table 'cperl6-mode-abbrev-table '(
		("if" "if" cperl6-electric-keyword 0)
		("elsif" "elsif" cperl6-electric-keyword 0)
		("while" "while" cperl6-electric-keyword 0)
		("until" "until" cperl6-electric-keyword 0)
		("unless" "unless" cperl6-electric-keyword 0)
		("else" "else" cperl6-electric-else 0)
		("continue" "continue" cperl6-electric-else 0)
		("for" "for" cperl6-electric-keyword 0)
		("foreach" "foreach" cperl6-electric-keyword 0)
		("formy" "formy" cperl6-electric-keyword 0)
		("foreachmy" "foreachmy" cperl6-electric-keyword 0)
		("do" "do" cperl6-electric-keyword 0)
		("=pod" "=pod" cperl6-electric-pod 0)
		("=over" "=over" cperl6-electric-pod 0)
		("=head1" "=head1" cperl6-electric-pod 0)
		("=head2" "=head2" cperl6-electric-pod 0)
		("pod" "pod" cperl6-electric-pod 0)
		("over" "over" cperl6-electric-pod 0)
		("head1" "head1" cperl6-electric-pod 0)
		("head2" "head2" cperl6-electric-pod 0)))
	(setq abbrevs-changed prev-a-c)))
  (setq local-abbrev-table cperl6-mode-abbrev-table)
  (if (cperl6-val 'cperl6-electric-keywords)
      (abbrev-mode 1))
  (set-syntax-table cperl6-mode-syntax-table)
  ;; Until Emacs is multi-threaded, we do not actually need it local:
  (make-local-variable 'cperl6-font-lock-multiline-start)
  (make-local-variable 'cperl6-font-locking)
  (make-local-variable 'outline-regexp)
  ;; (setq outline-regexp imenu-example--function-name-regexp-perl)
  (setq outline-regexp cperl6-outline-regexp)
  (make-local-variable 'outline-level)
  (setq outline-level 'cperl6-outline-level)
  ...
  (setq indent-line-function 'cperl6-indent-line)
  ...
  (setq comment-column cperl6-comment-column)
  (make-local-variable 'comment-start-skip)
  (setq comment-start-skip "#+ *")
  (make-local-variable 'defun-prompt-regexp)
;;;       "[ \t]*sub"
;;;	  (cperl6-after-sub-regexp 'named nil) ; 11=name 14=proto 17=attr-start
;;;	  cperl6-maybe-white-and-comment-rex	; 16=pre-block
  (setq defun-prompt-regexp
	(concat "[ \t]*sub"
		(cperl6-after-sub-regexp 'named 'attr-groups)
		cperl6-maybe-white-and-comment-rex))
  (make-local-variable 'comment-indent-function)
  (setq comment-indent-function 'cperl6-comment-indent)
  (and (boundp 'fill-paragraph-function)
      (progn
	(make-local-variable 'fill-paragraph-function)
	(set 'fill-paragraph-function 'cperl6-fill-paragraph)))
  (make-local-variable 'parse-sexp-ignore-comments)
  (setq parse-sexp-ignore-comments t)
  (make-local-variable 'indent-region-function)
  (setq indent-region-function 'cperl6-indent-region)
  ;;(setq auto-fill-function 'cperl6-do-auto-fill) ; Need to switch on and off!
  (make-local-variable 'imenu-create-index-function)
  (setq imenu-create-index-function
	(function cperl6-imenu--create-perl-index))
  (make-local-variable 'imenu-sort-function)
  (setq imenu-sort-function nil)
  (make-local-variable 'vc-rcs-header)
  (set 'vc-rcs-header cperl6-vc-rcs-header)
  (make-local-variable 'vc-sccs-header)
  (set 'vc-sccs-header cperl6-vc-sccs-header)
  ;; This one is obsolete...
  (make-local-variable 'vc-header-alist)
  (set 'vc-header-alist (or cperl6-vc-header-alist ; Avoid warning
			    (` ((SCCS (, (car cperl6-vc-sccs-header)))
				     (RCS (, (car cperl6-vc-rcs-header)))))))
  (cond ((boundp 'compilation-error-regexp-alist-alist);; xemacs 20.x, emacs22
	 (make-local-variable 'compilation-error-regexp-alist-alist)
	 (set 'compilation-error-regexp-alist-alist
	      (cons (cons 'cperl6 cperl6-compilation-error-regexp-alist)
	...
	(cond
	 ...
	 ((string< emacs-version "19.33") ; Which one to use?
	  '((perl-font-lock-keywords
	     ...
	     perl-font-lock-keywords-2) nil nil ((?_ . "w"))))
	 (t
	  '((cperl6-load-font-lock-keywords
	     cperl6-load-font-lock-keywords-1
	     cperl6-load-font-lock-keywords-2) nil nil ((?_ . "w"))))))
  (make-local-variable 'cperl6-syntax-state)
  (setq cperl6-syntax-state nil)		; reset syntaxification cache
  (if cperl6-use-syntax-table-text-property
      (progn
	...
	(unless cperl6-xemacs-p		; Our: just a plug for wrong font-lock
	  (make-local-variable 'font-lock-unfontify-region-function)
	  (set 'font-lock-unfontify-region-function ; not present with old Emacs
	       'cperl6-font-lock-unfontify-region-function))
	(make-local-variable 'cperl6-syntax-done-to)
	(setq cperl6-syntax-done-to nil)	; reset syntaxification cache
	...
	(setq font-lock-syntactic-keywords
		  ...
		  '(t (cperl6-fontify-syntaxically))
		'(t)))))
  (if (boundp 'font-lock-multiline)	; Newer font-lock; use its facilities
      (progn
	(setq cperl6-font-lock-multiline t) ; Not localized...
    ...
    (set 'font-lock-fontify-region-function ; not present with old Emacs
	 'cperl6-font-lock-fontify-region-function))
  (make-local-variable 'font-lock-fontify-region-function)
  (set 'font-lock-fontify-region-function ; not present with old Emacs
       'cperl6-font-lock-fontify-region-function)
  (make-local-variable 'cperl6-old-style)
  (make-local-variable 'cperl6-use-v6)
  (if (cperl6-looks-like-perl6)
      (set 'cperl6-use-v6 t)
    (set 'cperl6-use-v6 nil)
    )
  (if (boundp 'normal-auto-fill-function) ; 19.33 and later
      (set (make-local-variable 'normal-auto-fill-function)
	   'cperl6-do-auto-fill)	      ; RMS has it as #'cperl6-do-auto-fill ???
    (or (fboundp 'cperl6-old-auto-fill-mode)
	(progn
	  (fset 'cperl6-old-auto-fill-mode (symbol-function 'auto-fill-mode))
	  (defun auto-fill-mode (&optional arg)
	    (interactive "P")
	    (eval '(cperl6-old-auto-fill-mode arg)) ; Avoid a warning
	    (and auto-fill-function (memq major-mode '(perl-mode cperl6-mode))
		 (setq auto-fill-function 'cperl6-do-auto-fill))))))
  (if (cperl6-enable-font-lock)
      (if (cperl6-val 'cperl6-font-lock)
	  (progn (or cperl6-faces-init (cperl6-init-faces))
		 (font-lock-mode 1))))
  (set (make-local-variable 'facemenu-add-face-function)
       'cperl6-facemenu-add-face-function) ; XXXX What this guy is for???
  (and (boundp 'msb-menu-cond)
       (not cperl6-msb-fixed)
       (cperl6-msb-fix))
  (if (featurep 'easymenu)
      (easy-menu-add cperl6-menu))	; A NOP in RMS Emacs.
  (run-hooks 'cperl6-mode-hook)
  (if cperl6-hook-after-change
      (progn
	(make-local-hook 'after-change-functions)
	(add-hook 'after-change-functions 'cperl6-after-change-function nil t)))
  ;; After hooks since fontification will break this
  (if cperl6-pod-here-scan
      (or cperl6-syntaxify-by-font-lock
       (progn (or cperl6-faces-init (cperl6-init-faces-weak))
	      (cperl6-find-pods-heres)))))

...
(defun cperl6-msb-fix ()
  ...
  (let* ((l (length msb-menu-cond))
    ...
    (setcdr precdr (list
		    (list
		     '(memq major-mode '(cperl6-mode perl-mode))
		     handle
		     "Perl Files (%d)")
		    last))))

;; This is used by indent-for-comment
;; to decide how much to indent a comment in Cperl6 code
;; based on its context.  Do fallback if comment is found wrong.

(defvar cperl6-wrong-comment)
(defvar cperl6-st-cfence '(14))		; Comment-fence
(defvar cperl6-st-sfence '(15))		; String-fence
(defvar cperl6-st-punct '(1))
(defvar cperl6-st-word '(2))
(defvar cperl6-st-bra '(4 . ?\>))
(defvar cperl6-st-ket '(5 . ?\<))


(defun cperl6-comment-indent ()
  (let ((p (point)) (c (current-column)) was phony)
    (if (looking-at "^#") 0		; Existing comment at bol stays there.
      ;; Wrong comment found
      (save-excursion
	(setq was (cperl6-to-comment-or-eol)
	...
	(if (= (point) p)
	  ...
	  (setq cperl6-wrong-comment t)
	  (cperl6-make-indent comment-column 1 'keep)	; Indent minimum 1
	  c)))))			; except leave at least one space.

;;;(defun cperl6-comment-indent-fallback ()
...
(defun cperl6-indent-for-comment ()
  "Substitute for `indent-for-comment' in Cperl6."
  (interactive)
  (let (cperl6-wrong-comment)
    (indent-for-comment)
    (if cperl6-wrong-comment		; set by `cperl6-comment-indent'
	(progn (cperl6-to-comment-or-eol)
	       (forward-char (length comment-start))))))

(defun cperl6-comment-region (b e arg)
  "Comment or uncomment each line in the region in Cperl6 mode.
See `comment-region'."
  (interactive "r\np")
  (let ((comment-start "#"))
    (comment-region b e arg)))

(defun cperl6-uncomment-region (b e arg)
  "Uncomment or comment each line in the region in Cperl6 mode.
See `comment-region'."
  (interactive "r\np")
  (let ((comment-start "#"))
    (comment-region b e (- arg))))

(defvar cperl6-brace-recursing nil)

(defun cperl6-electric-brace (arg &optional only-before)
  "Insert character and correct line's indentation.
If ONLY-BEFORE and `cperl6-auto-newline', will insert newline before the
place (even in empty line), but not after.  If after \")\" and the inserted
char is \"{\", insert extra newline before only if
`cperl6-extra-newline-before-brace'."
  (interactive "P")
  (let (insertpos
	(other-end (if (and cperl6-electric-parens-mark
			    (cperl6-mark-active)
			    (< (mark) (point)))
		       (mark)
		     nil)))
    (if (and other-end
	     (not cperl6-brace-recursing)
	     (cperl6-val 'cperl6-electric-parens)
	     (>= (save-excursion (cperl6-to-comment-or-eol) (point)) (point)))
	...
	(progn
	  (save-excursion
	    ...
	    (cperl6-electric-lbrace arg insertpos))
	  (forward-char 1))
      ...
      (if (and (eq last-command-char ?\})
		...
		(condition-case nil
		    (save-excursion
		      ...
		      ;;(cperl6-after-block-p (point-min))
		      (or (cperl6-after-expr-p nil "{;)")
			  ;; after sub, else, continue
			  (cperl6-after-block-p nil 'pre)))
		  (error nil))))
	  ;; Just insert the guy
	...
	(if (and (not arg)		; No args, end (of empty line or auto)
		 ...
		 (or (and (null only-before)
		     ...
		     (and (eq last-command-char ?\{) ; Do not insert newline
			  ;; if after ")" and `cperl6-extra-newline-before-brace'
			  ;; is nil, do not insert extra newline.
			  (not cperl6-extra-newline-before-brace)
			  (save-excursion
			    (skip-chars-backward " \t")
			    (eq (preceding-char) ?\))))
		     (if cperl6-auto-newline
			 (progn (cperl6-indent-line) (newline) t) nil)))
	    (progn
	      (self-insert-command (prefix-numeric-value arg))
	      (cperl6-indent-line)
	      (if cperl6-auto-newline
		  (setq insertpos (1- (point))))
	      (if (and cperl6-auto-newline (null only-before))
		  (progn
		    (newline)
		    (cperl6-indent-line)))
	      (save-excursion
...
(defun cperl6-electric-lbrace (arg &optional end)
  "Insert character, correct line's indentation, correct quoting by space."
  (interactive "P")
  (let ((cperl6-brace-recursing t)
	(cperl6-auto-newline cperl6-auto-newline)
	(other-end (or end
		       (if (and cperl6-electric-parens-mark
				(cperl6-mark-active)
				(> (mark) (point)))
			   (save-excursion
			     (goto-char (mark))
			 ...
			 nil)))
	pos after)
    (and (cperl6-val 'cperl6-electric-lbrace-space)
	 (eq (preceding-char) ?$)
	 (save-excursion
	   (skip-chars-backward "$")
	   (looking-at "\\(\\$\\$\\)*\\$\\([^\\$]\\|$\\)"))
    ...
    (if (and
	 (save-excursion
	   (beginning-of-line)
	   (not (looking-at "[ \t]*#")))
	 (cperl6-after-expr-p nil "{;)"))
	nil
      (setq cperl6-auto-newline nil))
    (cperl6-electric-brace arg)
    (and (cperl6-val 'cperl6-electric-parens)
	 (eq last-command-char ?{)
	 (memq last-command-char
	       (append cperl6-electric-parens-string nil))
	 (or (if other-end (goto-char (marker-position other-end)))
	     t)
	 (setq last-command-char ?} pos (point))
	 (progn (cperl6-electric-brace arg t)
		(goto-char pos)))))

(defun cperl6-electric-paren (arg)
  "Insert an opening parenthesis or a matching pair of parentheses.
See `cperl6-electric-parens'."
  (interactive "P")
  (let ((beg (save-excursion (beginning-of-line) (point)))
	(other-end (if (and cperl6-electric-parens-mark
			 ...
			 (point-marker))
		     nil)))
    (if (and (cperl6-val 'cperl6-electric-parens)
	     (memq last-command-char
		   (append cperl6-electric-parens-string nil))
	     (>= (save-excursion (cperl6-to-comment-or-eol) (point)) (point))
...
(defun cperl6-electric-rparen (arg)
...
Affected by `cperl6-electric-parens'."
  (interactive "P")
  (let ((beg (save-excursion (beginning-of-line) (point)))
	(other-end (if (and cperl6-electric-parens-mark
			    (cperl6-val 'cperl6-electric-parens)
...
(defun cperl6-electric-keyword ()
  "Insert a construction appropriate after a keyword.
Help message may be switched off by setting `cperl6-message-electric-keyword'
to nil."
  (let ((beg (save-excursion (beginning-of-line) (point)))
    ...
    (and (save-excursion
	 ...
	 (save-excursion (or (not (re-search-backward "^=" nil t))
			     (or
			      (looking-at "=cut")
			      (and cperl6-use-syntax-table-text-property
	 ...
	 (progn
	   ...
	   (if dollar (progn (search-backward "$")
		 ...
		 (progn			; Avoid "if (())"
		   ...
		   (delete-backward-char -1))))
	   (if delete
	       (cperl6-putback-char cperl6-del-back-ch))
	   (if cperl6-message-electric-keyword
	       (message "Precede char by C-q to avoid expansion"))))))

(defun cperl6-ensure-newlines (n &optional pos)
  ...
  (if (looking-at "\n")
    ...
    (insert "\n"))
  (if (> n 1)
      (cperl6-ensure-newlines (1- n) pos)
    (goto-char pos)))

(defun cperl6-electric-pod ()
	   ...
	   (and
	    ...
	    (or
	     ...
	     (and (re-search-backward "\\(\\`\n?\\|^\n\\)=\\sw+" (point-min) t)
		  (not (looking-at "\n*=cut"))
		  (or (not cperl6-use-syntax-table-text-property)
  ...
  (let ((beg (save-excursion (beginning-of-line) (point))))
    (and (save-excursion
	 ...
	 (progn
	   ...
	   (cperl6-indent-line)
	   (cperl6-putback-char cperl6-del-back-ch)
	   (setq this-command 'cperl6-electric-else)
	   (if cperl6-message-electric-keyword
	       (message "Precede char by C-q to avoid expansion"))))))

(defun cperl6-linefeed ()
...
If in POD, insert appropriate lines."
  ...
  (let ((beg (save-excursion (beginning-of-line) (point)))
    ...
    (if (and				; Check if we need to split:
	 ...
	 (save-excursion (cperl6-to-comment-or-eol)
	...
	(if (and over
		 (progn
		   ...
		   res))
	    nil
	  (cperl6-ensure-newlines (if cut 2 4))
	  (forward-line 2)))
...
(defun cperl6-electric-semi (arg)
  "Insert character and correct line's indentation."
  (interactive "P")
  (if cperl6-auto-newline
      (cperl6-electric-terminator arg)
    (self-insert-command (prefix-numeric-value arg))
    (if cperl6-autoindent-on-semi
	(cperl6-indent-line))))

(defun cperl6-electric-terminator (arg)
  "Insert character and correct line's indentation."
  (interactive "P")
  (let ((end (point))
	(auto (and cperl6-auto-newline
...
(defun cperl6-electric-backspace (arg)
  "Backspace, or remove the whitespace around the point inserted by an electric
key.  Will untabivy if `cperl6-electric-backspace-untabify' is non-nil."
  (interactive "p")
  (if (and cperl6-auto-newline
	   (memq last-command '(cperl6-electric-semi
				cperl6-electric-terminator
				cperl6-electric-lbrace))
	   (memq (preceding-char) '(?\  ?\t ?\n)))
      (let (p)
	(if (eq last-command 'cperl6-electric-lbrace)
	    (skip-chars-forward " \t\n"))
	(setq p (point))
	(skip-chars-backward " \t\n")
	(delete-region (point) p))
    (and (eq last-command 'cperl6-electric-else)
	 ;; We are removing the whitespace *inside* cperl6-electric-else
	 (setq this-command 'cperl6-electric-else-really))
    (if (and cperl6-auto-newline
	     (eq last-command 'cperl6-electric-else-really)
	     (memq (preceding-char) '(?\  ?\t ?\n)))
	(let (p)
	  (skip-chars-forward " \t\n")
	  (setq p (point))
	  (skip-chars-backward " \t\n")
	  (delete-region (point) p))
      (if cperl6-electric-backspace-untabify
	  (backward-delete-char-untabify arg)
	(delete-backward-char arg)))))

(defun cperl6-inside-parens-p ()		;; NOT USED????
  (condition-case ()
      (save-excursion
	(save-restriction
    ...
    (error nil)))

(defun cperl6-indent-command (&optional whole-exp)
  "Indent current line as Perl code, or in some cases insert a tab character.
If `cperl6-tab-always-indent' is non-nil (the default), always indent current
...
The relative indentation among the lines of the expression are preserved."
  ...
  (if whole-exp
      ...
      (let ((shift-amt (cperl6-indent-line))
	    beg end)
	(save-excursion
	  (if cperl6-tab-always-indent
	      (beginning-of-line))
	  (setq beg (point))
	  (forward-sexp 1)
	  (setq end (point))
    ...
    (if (and (not cperl6-tab-always-indent)
	     (save-excursion
	       (skip-chars-backward " \t")
	       (not (bolp))))
	(insert-tab)
...
(defun cperl6-indent-line (&optional parse-data)
  "Indent current line as Perl code.
Return the amount the indentation changed by."
  (let ((case-fold-search nil)
	(pos (- (point-max) (point)))
	indent i beg shift-amt)
    (setq indent (cperl6-calculate-indent parse-data)
	  i indent)
    (beginning-of-line)
    (setq beg (point))
    (cond ((or (eq indent nil) (eq indent t))
	  ...
	  ;;((eq indent t)    ; Never?
	  ;; (setq indent (cperl6-calculate-indent-within-comment)))
	  ;;((looking-at "[ \t]*#")
	  ;; (setq indent 0))
	  (t
	   (skip-chars-forward " \t")
	   ...
	   (cond ((looking-at "[A-Za-z_][A-Za-z_0-9]*:[^:]")
		  (and (> indent 0)
		       (setq indent (max cperl6-min-label-indent
					 (+ indent cperl6-label-offset)))))
		 ((= (following-char) ?})
		  (setq indent (- indent cperl6-indent-level)))
		 ((memq (following-char) '(?\) ?\])) ; To line up with opening paren.
		  (setq indent (+ indent cperl6-close-paren-offset)))
		 ((= (following-char) ?{)
		  (setq indent (+ indent cperl6-brace-offset))))))
    (skip-chars-forward " \t")
...
(defun cperl6-get-state (&optional parse-start start-state)
...
(defun cperl6-calculate-indent (&optional parse-data) ; was parse-start
...
and closing parentheses and brackets."
  ...
  (save-excursion
    (if (or
	 ...
	 (and (not cperl6-indent-left-aligned-comments)
	      (looking-at "^#")))
	nil
      (beginning-of-line)
	...
	(cond
	 ...
	 (t
	  (save-excursion
	    ;; Not in POD
	    (cperl6-backward-to-noncomment nil)
	    (setq p (max (point-min) (1- (point)))
		  prop (get-text-property p 'syntax-type)
		  look-prop (or (nth 1 (assoc prop cperl6-look-for-prop))
				'syntax-type))
	    (if (memq prop '(pod here-doc format here-doc-delim))
		(progn
		  (goto-char (cperl6-beginning-of-property p look-prop))
		  (beginning-of-line)
		  (setq pre-indent-point (point)))))))
	(goto-char pre-indent-point)
	(let* ((case-fold-search nil)
	       (s-s (cperl6-get-state (car parse-data) (nth 1 parse-data)))
	  ...
	  (cond ((get-text-property (point) 'indentable)
		 ...
		 ;; (same offset as `cperl6-beautify-regexp-piece'),
		 ;; skip blanks if we do not close the expression.
		 (setq delim		; We do not close the expression
		       (get-text-property
			(cperl6-1+ char-after-pos) 'indentable)
		       p (1+ (cperl6-beginning-of-property
			      (point) 'indentable))
		       is-block		; misused for: preceeding line in REx
		       (save-excursion	; Find preceeding line
			 (cperl6-backward-to-noncomment p)
			 (beginning-of-line)
			 (if (<= (point) p)
		 ...
		 (cond ((not delim)
			...
			(goto-char is-block)
			(+ (* (nth 0 p)
			      (or cperl6-regexp-indent-step cperl6-indent-level))
			   (cond ((eq char-after ?\) )
				  (- cperl6-close-paren-offset)) ; compensate
				 ((eq char-after ?\| )
				  (- (or cperl6-regexp-indent-step cperl6-indent-level)))
				 (t 0))
		...
		((null containing-sexp)
		 ...
		 (+ (save-excursion
		      (goto-char start)
		      (- (current-indentation)
			 (if (nth 2 s-s) cperl6-indent-level 0)))
		    (if (eq char-after ?{) cperl6-continued-brace-offset 0)
		    (progn
		      (cperl6-backward-to-noncomment (or old-indent (point-min)))
		...
		((not
		 ...
		 (+ (current-column)
			...
			(+ cperl6-indent-level cperl6-close-paren-offset)
		      0)))
;;;	      ((and (/= (char-after containing-sexp) ?{)
;;;		    (not cperl6-indent-parens-as-block))
...
;;;		  (if (eq char-after ?\}) (+ cperl6-indent-level
;;;					     cperl6-close-paren-offset)
;;;		    0)))
		(t
		 ;; Statement level.  Is it a continuation or a new statement?
		 ;; Find previous non-comment character.
		 (goto-char pre-indent-point)
		 (cperl6-backward-to-noncomment containing-sexp)
		 ...
		 (while;;(or (eq (preceding-char) ?\,)
		   ...
		   (if (eq (preceding-char) ?\,)
		       ...
		       (cperl6-backward-to-start-of-continued-exp containing-sexp))
		   (beginning-of-line)
		   (cperl6-backward-to-noncomment containing-sexp))
		 ...
		 (if (not (or (eq (1- (point)) containing-sexp)
		     ...
		     ;; indent  `cperl6-continued-statement-offset'  more than the
		     ...
		     (progn
			  ...
			  ;; Labels do not take :: ...
			  (if (looking-at "\\(\\w\\|_\\)+[ \t]*:")
			      (if (> (current-indentation) cperl6-min-label-indent)
				  (- (current-indentation) cperl6-label-offset)
				;; Do not move `parse-data', this should
				;; be quick anyway (this comment comes
				;; from different location):
				(cperl6-calculate-indent))
			    (current-column))
			  (if (eq char-after ?\{)
		   ...
		   (or
			...
			(and (< (point) indent-point)
				 ...
				 (if (> (current-indentation)
				   ...
				   (+ old-indent cperl6-indent-level))
			       (current-column)))))
		    ;; If no previous statement,
		    ;; indent it relative to line brace is on.
		    ;; For open brace in column zero, don't let statement
		    ;; start there too.  If cperl6-indent-level is zero,
		    ;; use cperl6-brace-offset + cperl6-continued-statement-offset instead.
		    ;; For open-braces not the first thing in a line,
		    ;; add in cperl6-brace-imaginary-offset.

		    ;; If first thing on a line:  ?????
		    (+ (if (and (bolp) (zerop cperl6-indent-level))
			   (+ cperl6-brace-offset cperl6-continued-statement-offset)
			 cperl6-indent-level)
		       (if (or is-block
			       (not delim)
			 ...
			 (+ cperl6-indent-level cperl6-close-paren-offset))
		       ...
		       ;; add the cperl6-brace-imaginary-offset.
		       (progn (skip-chars-backward " \t")
			      (if (bolp) 0 cperl6-brace-imaginary-offset))
		       ;; If the openbrace is preceded by a parenthesized exp,
		       ;; move to the beginning of that;
		       ;; possibly a different line
		       (progn
			 (cperl6-backward-to-noncomment (point-min))
			 (if (eq (preceding-char) ?\))
			     (forward-sexp -1))
...
(defvar cperl6-indent-alist
  '((string nil)
    ...
    (expression 1))
  "Alist of indentation rules for Cperl6 mode.
The values mean:
  nil: do not indent;
...
(defun cperl6-where-am-i (&optional parse-start start-state)
  ;; Unfinished
    ...
    (let* ((start-point (point)) unused
      ...
      (cond ((nth 3 state)		; In string
	    ...
	    ((null containing-sexp)
	     ...
	     ;; unless that ends in a closeparen without semicolon,
	     ;; in which case this line is the first argument decl.
	     (cperl6-backward-to-noncomment (or parse-start (point-min)))
	    ...
	    (t
	     ...
	     (while (or (eq (preceding-char) ?\,)
			(save-excursion (cperl6-after-label)))
	       (if (eq (preceding-char) ?\,)
		   ;; Will go to beginning of line, essentially
		   ;; Will ignore embedded sexpr XXXX.
		   (cperl6-backward-to-start-of-continued-exp containing-sexp))
...
(defun cperl6-calculate-indent-within-comment ()
...
the current line is to be regarded as part of a block comment."
  (let (end star-start)
    (save-excursion
      ...
      (and (= (following-char) ?#)
	   (forward-line -1)
	   (cperl6-to-comment-or-eol)
	   (setq end (point)))
      (goto-char end)
      (current-column))))


(defun cperl6-to-comment-or-eol ()
...
Returns true if comment is found.  In POD will not move the point."
  ...
  (let (state stop-in cpoint (lim (progn (end-of-line) (point))) pr e)
    (or cperl6-font-locking
	(cperl6-update-syntaxification lim lim))
    (beginning-of-line)
...
(defsubst cperl6-modify-syntax-type (at how)
  (if (< at (point-max))
      (progn
	(put-text-property at (1+ at) 'syntax-table how)
	(put-text-property at (1+ at) 'rear-nonsticky '(syntax-table)))))

(defun cperl6-protect-defun-start (s e)
  ;; C code looks for "^\\s(" to skip comment backward in "hard" situations
  (save-excursion
    (goto-char s)
    (while (re-search-forward "^\\s(" e 'to-end)
      (put-text-property (1- (point)) (point) 'syntax-table cperl6-st-punct))))

(defun cperl6-commentify (bb e string &optional noface)
  (if cperl6-use-syntax-table-text-property
      (if (eq noface 'n)		; Only immediate
	  nil
	;; We suppose that e is _after_ the end of construction, as after eol.
	(setq string (if string cperl6-st-sfence cperl6-st-cfence))
	(if (> bb (- e 2))
	    ;; one-char string/comment?!
	    (cperl6-modify-syntax-type bb cperl6-st-punct)
	  (cperl6-modify-syntax-type bb string)
	  (cperl6-modify-syntax-type (1- e) string))
	(if (and (eq string cperl6-st-sfence) (> (- e 2) bb))
	    (put-text-property (1+ bb) (1- e)
			       'syntax-table cperl6-string-syntax-table))
	(cperl6-protect-defun-start bb e))
    ;; Fontify
    (or noface
	(not cperl6-pod-here-fontify)
	(put-text-property bb e 'face (if string 'font-lock-string-face
					'font-lock-comment-face)))))

(defvar cperl6-starters '(( ?\( . ?\) )
			 ( ?\[ . ?\] )
			 ( ?\{ . ?\} )
			 ( ?\< . ?\> )))

(defun cperl6-cached-syntax-table (st)
...
(defun cperl6-forward-re (lim end is-2arg st-l err-l argument
...
modify syntax-type text property if the situation is too hard."
  (let (b starter ender st i i2 go-forward reset-st set-st)
    ...
    (setq b (point)
	  ...
	  ender (cdr (assoc starter cperl6-starters)))
    ;; What if starter == ?\\  ????
    (setq st (cperl6-cached-syntax-table st-l))
    (setq set-st t)
	...
	(progn
	  ...
	  (if (and (eq starter (char-after (cperl6-1+ b)))
		   (not ender))
	  ...
	  (and is-2arg			; Have trailing part
		 ...
		 (or (eq (char-syntax (following-char)) ?.)
		     ...
		     (cperl6-modify-syntax-type (point) cperl6-st-punct))
	  ...
	  (if is-2arg			; Not number => have second part
		...
		(setq set-st nil)
		(setq ender (cperl6-forward-re lim end nil st-l err-l
...
(defun cperl6-forward-group-in-re (&optional st-l)
...
Works before syntax recognition is done."
  ...
  (let (st b reset-st)
	...
	(progn
	  ...
	  (forward-sexp 1))
      (error (message
	      "cperl6-forward-group-in-re: error %s" b)))
...
(defsubst cperl6-postpone-fontification (b e type val &optional now)
  ;; Do after syntactic fontification?
  (if cperl6-syntaxify-by-font-lock
      (or now (put-text-property b e 'cperl6-postpone (cons type val)))
...
(defun cperl6-unwind-to-safe (before &optional end)
  ...
  (let ((pos (point)) opos)
    (while (and pos (progn
      ...
      (setq opos pos
	    pos (cperl6-beginning-of-property pos 'syntax-type))
      (if (eq pos (point-min))
	  (setq pos nil))
      (if pos
	  (if before
	      (progn
		(goto-char (cperl6-1- pos))
		(beginning-of-line)
		(setq pos (point)))
	    (goto-char (setq pos (cperl6-1- pos))))
	;; Up to the start
	(goto-char (point-min))))
    ;; Skip empty lines
    (and (looking-at "\n*=")
...
(defvar cperl6-nonoverridable-face)
...
(defun cperl6-find-sub-attrs (&optional st-l b-fname e-fname pos)
...
Should be called with the point before leading colon of an attribute."
  ...
  (let (st b p reset-st after-first (start (point)) start1 end1)
	...
	(while (looking-at
		(concat
		 "\\("			; 1=optional? colon
		   ":" cperl6-maybe-white-and-comment-rex ; 2=whitespace/comment?
		 "\\)"
		 (if after-first "?" "")
	  ...
	  (and (match-beginning 1)
		...
		(match-beginning 0) (cperl6-1+ (match-beginning 0))
		'face font-lock-constant-face))
	  (setq start1 (match-beginning 3) end1 (match-end 3))
	  (cperl6-postpone-fontification start1 end1
					'face font-lock-constant-face)
	  ...
	  (if (match-end 4)		; Have attribute arguments...
		...
		(set-syntax-table reset-st)
		(setq reset-st nil)
		(cperl6-commentify p (point) t))) ; mark as string
    ...
    (and start
	 (progn
	   ...
	   (and pos
		...
		(put-text-property (+ 3 pos) (cperl6-1+ (point))
				   'syntax-type 'sub-decl))
	   (and b-fname			; Fontify here: the following condition
		(cperl6-postpone-fontification ; is too hard to determine by
...
(defsubst cperl6-look-at-leading-count (is-x-REx e)
...
(defun cperl6-find-pods-heres (&optional min max non-inter end ignore-max end-of-here-doc)
  "Scans the buffer for hard-to-parse Perl constructions.
If `cperl6-pod-here-fontify' is not-nil after evaluation, will fontify
the sections using `cperl6-pod-head-face', `cperl6-pod-face',
`cperl6-here-face'."
  (interactive)
 (or min (setq min (point-min)
		cperl6-syntax-state nil
		cperl6-syntax-done-to min))
  (or max (setq max (point-max)))
  (let* ((cperl6-pod-here-fontify (eval cperl6-pod-here-fontify)) go tmpend
	 ...
	 (cperl6-font-locking t)
	 (use-syntax-state (and cperl6-syntax-state
				(>= min (car cperl6-syntax-state))))
	 (state-point (if use-syntax-state
			  (car cperl6-syntax-state)
			(point-min)))
	 (state (if use-syntax-state
		    (cdr cperl6-syntax-state)))
	 ...
	 (font-lock-string-face (if (boundp 'font-lock-string-face)
				  ...
				  'font-lock-string-face))
	 (my-cperl6-delimiters-face (if (boundp 'font-lock-constant-face)
				      font-lock-constant-face
				    'font-lock-constant-face))
	 (my-cperl6-REx-spec-char-face	; [] ^.$ and wrapper-of ({})
	 ...
	 (font-lock-variable-name-face	; interpolated vars and ({})-code
	  (if (boundp 'font-lock-variable-name-face)
	      font-lock-variable-name-face
	    'font-lock-variable-name-face))
	 (font-lock-function-name-face	; used in `cperl6-find-sub-attrs'
	  (if (boundp 'font-lock-function-name-face)
	      font-lock-function-name-face
	    'font-lock-function-name-face))
	 (font-lock-constant-face	; used in `cperl6-find-sub-attrs'
	  (if (boundp 'font-lock-constant-face)
	      font-lock-constant-face
	    'font-lock-constant-face))
	 (my-cperl6-REx-0length-face ; 0-length, (?:)etc, non-literal \
	 ...
	 (font-lock-warning-face
	  (if (boundp 'font-lock-warning-face)
	      font-lock-warning-face
	    'font-lock-warning-face))
	 (my-cperl6-REx-ctl-face		; (|)
	  (if (boundp 'font-lock-keyword-face)
	      font-lock-keyword-face
	    'font-lock-keyword-face))
	 (my-cperl6-REx-modifiers-face	; //gims
	  (if (boundp 'cperl6-nonoverridable-face)
	      cperl6-nonoverridable-face
	    'cperl6-nonoverridable-face))
	 (my-cperl6-REx-length1-face	; length=1 escaped chars, POSIX classes
	 ...
	 (search
	  (concat
	   ...
	   (if cperl6-use-syntax-table-text-property
		...
		(if (not cperl6-use-v6) ; perl6
		    "\\<\\(q[wxqr]?\\|[msy]\\|tr\\)\\>" ; QUOTED CONSTRUCT - Perl5
		  ;;"\\<\\(q[wxq]?\\)\\>" ; QUOTED CONSTRUCT - Perl6
                  "\\<\\(q[wxq]?\\|\\(?:rx\\|[ms]\\|tr\\)\\s *\\(?::\\(?:[igcpw]\\|ignorecase\\|global\\(?:ly\\)?\\|continue\\|pos\\|once\\|words\\|bytes\\|codes\\|graphs\\|langs\\|\\|[0-9]+\\(?:st\\|nd\\|rd\\|th\\|x\\)\\|ov\\|overlap\\|ex\\|exhaustive\\|rw\\|P5\\|perl5\\|Perl5\\(?:<[a-zA-Z]+>\\)?\\|nth\\(?:([0-9]+)\\)?\\|x\\(?:([0-9]+)\\)?\\)\\s *\\)*\\)\\>:?")            ; perl6: rx
                )
		"\\|"
		...
		;; "\\|"
		;; "\\<\\(\\(rx\\|[ms]\\|tr\\)\\s *\\(:\\([igcpw]\\|ignorecase\\|global\\(ly\\)?\\|continue\\|pos\\|once\\|words\\|bytes\\|codes\\|graphs\\|langs\\|\\|[0-9]+\\(st\\|nd\\|rd\\|th\\|x\\)\\|ov\\|overlap\\|ex\\|exhaustive\\|rw\\|P5\\|perl5\\|Perl5\\(<[a-zA-Z]+>\\)?\\|nth\\(([0-9]+)\\)?\\|x\\(([0-9]+)\\)?\\)\\s *\\)*\\)\\>:?")            ; perl6: rx
	...
	(progn
	  (save-excursion
	    ...
	    (and cperl6-pod-here-fontify
		 ;; We had evals here, do not know why...
		 (setq face cperl6-pod-face
		       head-face cperl6-pod-head-face
		       here-face cperl6-here-face))
		...
		(if (eq (preceding-char) ?\') ; $'
		    (progn
			  ...
			  (cperl6-modify-syntax-type (1- b) cperl6-st-punct))
		      (goto-char (1+ b)))
		  ;; else: ${
		...
		(if (or (nth 3 state) (nth 4 state)
		  ...
		  (while (re-search-forward "\n\n[ \t]" e t)
		    ...
		    (beginning-of-line)
		    (put-text-property (cperl6-1- b) (point) 'syntax-type 'pod)
		    (cperl6-put-do-not-fontify b (point) t)
		    ;; mark the non-literal parts as PODs
		    (if cperl6-pod-here-fontify
			(cperl6-postpone-fontification b (point) 'face face t))
		    (re-search-forward "\n\n[^ \t\f\n]" e 'toend)
		    (beginning-of-line)
		    (setq b (point)))
		  (put-text-property (cperl6-1- (point)) e 'syntax-type 'pod)
		  (cperl6-put-do-not-fontify (point) e t)
		  (if cperl6-pod-here-fontify
		      (progn
			;; mark the non-literal parts as PODs
			(cperl6-postpone-fontification (point) e 'face face t)
			(goto-char bb)
			(if (looking-at
			     "=[a-zA-Z0-9_]+\\>[ \t]*\\(\\(\n?[^\n]\\)+\\)$")
			    ;; mark the headers
		...
		(if c			; Not here-doc
		  ...
		  (cond (cperl6-pod-here-fontify
			 ;; Highlight the starting delimiter
			 (cperl6-postpone-fontification
			  b1 e1 'face my-cperl6-delimiters-face)
			 (cperl6-put-do-not-fontify b1 e1 t)))
		  (forward-line)
		  (setq i (point))
		  (if end-of-here-doc
		  ...
		  (if cperl6-pod-here-fontify
			...
			(cperl6-postpone-fontification
			 ...
			 'face my-cperl6-delimiters-face)
			(cperl6-put-do-not-fontify b (match-end 0) t)
			;; Highlight the HERE-DOC
			(cperl6-postpone-fontification b (match-beginning 0)
						      'face here-face)))
		  (setq e1 (cperl6-1+ (match-end 0)))
		  ...
		  ;; the whole construct:
		  (put-text-property b (cperl6-1+ b) 'front-sticky '(syntax-type))
		  (cperl6-commentify b e1 nil)
		  (cperl6-put-do-not-fontify b (match-end 0) t)
		  ;; Cache the syntax info...
		  (setq cperl6-syntax-state (cons state-point state))
		  ;; ... and process the rest of the line...
		  (setq overshoot
			(elt		; non-inter ignore-max
			 (cperl6-find-pods-heres c i t end t e1) 1))
		...
		(if cperl6-pod-here-fontify
			...
			(cperl6-postpone-fontification b1 (point)
						      'face font-lock-string-face)
			(cperl6-commentify b1 (point) nil)
			(cperl6-put-do-not-fontify b1 (point) t))))
		  ;; We do not search to max, since we may be called from
		  ;; some hook of fontification, and max is random
		...
		(if (looking-at "^\\.$") ; ";" is not supported yet
		    (progn
		      ;; Highlight the ending delimiter
		      (cperl6-postpone-fontification (point) (+ (point) 2)
						    'face font-lock-string-face)
		      (cperl6-commentify (point) (+ (point) 2) nil)
		      (cperl6-put-do-not-fontify (point) (+ (point) 2) t))
...
;;; functions/builtins which expect an argument, but ...
		...
		(if bb
		  ...
		  (setq b (point)	; before the first delimiter
			...
			;; We do not search to max, since we may be called from
			;; some hook of fontification, and max is random
			i (cperl6-forward-re stop-point end
					    i2
					    st-l err-l argument)
		  ...
		  (if (null i)
			...
			(progn
			  ...
			  (and
			   ;; silent:
			   (car (cperl6-find-pods-heres b1 (1- (point)) t end))
			   ;; Error
			   (goto-char (1+ max)))
			  (if (and tag (eq (preceding-char) ?\>))
			      (progn
				(cperl6-modify-syntax-type (1- (point)) cperl6-st-ket)
				(cperl6-modify-syntax-type i cperl6-st-bra)))
			  (put-text-property b i 'syntax-type 'string)
			  (if is-x-REx
			      (put-text-property b i 'indentable t)))
		      (cperl6-commentify b1 (point) t)
		      (put-text-property b (point) 'syntax-type 'string)
		      (if is-x-REx
			  (put-text-property b i 'indentable t))
		      (if qtag
			  (cperl6-modify-syntax-type (1+ i) cperl6-st-punct))
		      (setq tail nil)))
		  ;; Now: tail: if the second part is non-matching without ///e
		  (if (eq (char-syntax (following-char)) ?w)
		      (progn
			(forward-word 1) ; skip modifiers s///s
			(if tail (cperl6-commentify tail (point) t))
			(cperl6-postpone-fontification
			 e1 (point) 'face my-cperl6-REx-modifiers-face)))
		  ;; Check whether it is m// which means "previous match"
		  ;; and highlight differently
		  (setq is-REx
			(and (string-match "^\\([sm]?\\|qr\\|rx\\)$" argument)
		  ...
		  (if (and is-REx
		    ...
		    (if (or i2		; Has 2 args
			    (and cperl6-fontify-m-as-s
				 (or
				  ;; (string-match "^\\(m\\|qr\\)$" argument)    ; perl6
				  (string-match "^\\(\\(rx\\|[msy]\\|tr\\)\\s *\\(:\\([igcpw]\\|ignorecase\\|global\\(ly\\)?\\|continue\\|pos\\|once\\|words\\|bytes\\|codes\\|graphs\\|langs\\|\\|[0-9]+\\(st\\|nd\\|rd\\|th\\|x\\)\\|ov\\|overlap\\|ex\\|exhaustive\\|rw\\|P5\\|perl5\\|Perl5\\(<[a-zA-Z]+>\\)?\\|nth\\(([0-9]+)\\)?\\|x\\(([0-9]+)\\)?\\)\\s *\\)*\\)$" argument)           ; perl6: rx
				  (and (eq 0 (length argument))
				       (not (eq ?\< (char-after b)))))))
			(progn
			  (cperl6-postpone-fontification
			   b (cperl6-1+ b) 'face my-cperl6-delimiters-face)
			  (cperl6-postpone-fontification
			   (1- e) e 'face my-cperl6-delimiters-face)))
		    (if (and is-REx cperl6-regexp-scan)
...
;;;m^a[\^b]c^ + m.a[^b]\.c.;
			(save-excursion
			  (goto-char (1+ b))
			  ;; First
			  (cperl6-look-at-leading-count is-x-REx e)
			  (setq hairy-RE
				(concat
				 (if is-x-REx
				     (if (eq (char-after b) ?\#)
			  ...
			  (while
				...
				(if (and (eq (char-after b) qtag)
				    ...
				    (progn
				      (if (and cperl6-use-syntax-table-text-property
					       (eq qtag ?\) ))
					  (put-text-property
					   REx-subgr-start (1- (point))
					   'syntax-table cperl6-st-punct))
				      (cperl6-postpone-fontification
				       (1- (point)) (point) 'face
					; \] can't appear below
				...
				(cond
				 ...
				 ((or (and (memq qtag (append "01234567" nil))
				  ...
				  (cperl6-postpone-fontification
				   ...
				   'face my-cperl6-REx-length1-face))))
			      (setq was-subgr nil)) ; We do stuff here
			     ((match-beginning 3) ; [charclass]
			      (forward-char 1)
			      (if (eq (char-after b) ?^ )
				  (and (eq (following-char) ?\\ )
				       (eq (char-after (cperl6-1+ (point)))
					   ?^ )
				       (forward-char 2))
				(and (eq (following-char) ?^ )
				     (forward-char 1)))
...
;;;				       "\\=\\(\\\\.\\|[^][\\\\]\\|\\[:\\^?\sw+:]\\|\\[[^:]\\)*]")
				...
				(if (save-excursion
			      ...
			      (cperl6-postpone-fontification
			       ...
			       'face my-cperl6-REx-spec-char-face)
			      (cperl6-postpone-fontification
			       (1- (point)) (point) 'face
			       my-cperl6-REx-spec-char-face)
			      (if (eq (char-after b) ?\] )
				  (cperl6-postpone-fontification
				   (- (point) 2) (1- (point))
				   'face my-cperl6-REx-0length-face))
			      (while tag
				(cperl6-postpone-fontification
				...
				(setq tag (cdr tag)))
			     ...
			     ((match-beginning 5) ; before (?{}) (??{})
			      ...
			      (if (or (setq qtag
					    (cperl6-forward-group-in-re st-l))
				      (and (>= (point) e)
		  ...
		  (if i2
		      (progn
			(cperl6-postpone-fontification
			 (1- e1) e1 'face my-cperl6-delimiters-face)
			(if (assoc (char-after b) cperl6-starters)
			    (progn
			      (cperl6-postpone-fontification
			       b1 (1+ b1) 'face my-cperl6-delimiters-face)
		...
		;;"\\<sub\\>\\("			;12
		;;   cperl6-white-and-comment-rex	;13
		;;   "\\([a-zA-Z_:'0-9]+\\)\\)?" ; name	;14
		;;"\\(" cperl6-maybe-white-and-comment-rex	;15,16
		;;   "\\(([^()]*)\\|:[^:]\\)\\)" ; 17:proto or attribute start
		(if (not cperl6-use-v6)  ; perl6: for perl6 we just use ordinary highlighting in sub params
		    (progn
			...
			(if (eq (char-after (match-beginning 17)) ?\( )
			    (progn
			      (cperl6-commentify ; Prototypes; mark as string
			       (match-beginning 17) (match-end 17) t)
			      (goto-char (match-end 0))
			      ;; Now look for attributes after prototype:
			      (forward-comment (buffer-size))
			      (and (looking-at ":[^:]")
				   (cperl6-find-sub-attrs st-l b1 e1 b)))
		...
		(put-text-property (1- bb) bb 'syntax-table cperl6-st-word)
		(goto-char bb))
	       ;; 1+6+2+1+1+6+1+1=19 extra () before this:
	       ;; "__\\(END\\|DATA\\)__"
		...
		(cperl6-modify-syntax-type b cperl6-st-punct)
		(goto-char bb))
	    ...
	    (setq cperl6-syntax-state (cons state-point state)
		  ...
		  cperl6-syntax-done-to (or tmpend (max (point) max)))
	    ;;(message "state-at=%s, done-to=%s" state-point cperl6-syntax-done-to)
	    )
...
(defun cperl6-find-pods-heres-region (min max)
  (interactive "r")
  (cperl6-find-pods-heres min max))

(defun cperl6-backward-to-noncomment (lim)
  ...
  (let (stop p pr)
    (while (and (not stop) (> (point) (or lim (point-min))))
	  ...
	  (cperl6-unwind-to-safe nil)
	(or (and (looking-at "^[ \t]*\\(#\\|$\\)")
		 (not (memq pr '(string prestring))))
	    (progn (cperl6-to-comment-or-eol) (bolp))
	    (progn
	      (skip-chars-backward " \t")
	      (if (< p (point)) (goto-char p))
	      (setq stop t)))))))

;; Used only in `cperl6-calculate-indent'...
(defun cperl6-block-p ()		   ; Do not C-M-q !  One string contains ";" !
  ...
  (or
   ...
   (progn
     ...
     (or (memq (preceding-char) (append ";){}$@&%\C-@" nil)) ; Or label!  \C-@ at bobp
         ...
         (save-excursion (cperl6-after-label))
         (get-text-property (cperl6-1- (point)) 'attrib-group)
         (and (memq (char-syntax (preceding-char)) '(?w ?_))
              (progn
                (backward-sexp)
                ;; sub {BLK}, print {BLK} $data, but NOT `bless', `return', `tr'
                (or (and (looking-at "[a-zA-Z0-9_:]+[ \t\n\f]*[{#]") ; Method call syntax
                         (not (looking-at "\\(bless\\|return\\|\\(\\(rx\\|[msy]\\|tr\\)\\s *\\(:\\([igcpw]\\|ignorecase\\|global\\(ly\\)?\\|continue\\|pos\\|once\\|words\\|bytes\\|codes\\|graphs\\|langs\\|\\|[0-9]+\\(st\\|nd\\|rd\\|th\\|x\\)\\|ov\\|overlap\\|ex\\|exhaustive\\|rw\\|P5\\|perl5\\|Perl5\\(<[a-zA-Z]+>\\)?\\|nth\\(([0-9]+)\\)?\\|x\\(([0-9]+)\\)?\\)\\s *\\)*\\):?\\)")) ;; perl6 23.06.2006
...
;;; What is the difference of (cperl6-after-block-p lim t) and (cperl6-block-p)?
;;; No save-excursion; condition-case ...  In (cperl6-block-p) the block
...
(defun cperl6-after-block-p (lim &optional pre-block)
...
(defun cperl6-after-expr-p (&optional lim chars test)
...
`}' is treated \"smartly\" if it is not in the list)."
  (let ((lim (or lim (point-min)))
	...
	(if (get-text-property (point) 'here-doc-group)
	    (progn
	      (goto-char
	       (cperl6-beginning-of-property (point) 'here-doc-group))
	      (beginning-of-line 0)))
	(if (get-text-property (point) 'in-pod)
	    (progn
	      (goto-char
	       (cperl6-beginning-of-property (point) 'in-pod))
	      (beginning-of-line 0)))
	(if (looking-at "^[ \t]*\\(#\\|$\\)") nil ; Only comment, skip
	  ;; Else: last iteration, or a label
	  (cperl6-to-comment-or-eol)	; Will not move past "." after a format
	  (skip-chars-backward " \t")
	  (if (< p (point)) (goto-char p))
	  (setq p (point))
	  ...
	  (progn
	    (if test (eval test)
		  ...
		  (and (eq (preceding-char) ?\})
		       (cperl6-after-block-p lim))
		  (and (eq (following-char) ?.)	; in format: see comment above
		       (eq (get-text-property (point) 'syntax-type)
			   'format)))))))))

(defun cperl6-backward-to-start-of-expr (&optional lim)
  (condition-case nil
      (progn
	(while (and (or (not lim)
			(> (point) lim))
		    (not (cperl6-after-expr-p lim)))
	  (forward-sexp -1)
	  ;; May be after $, @, $# etc of a variable
	  (skip-chars-backward "$@%#")))
    (error nil)))

(defun cperl6-at-end-of-expr (&optional lim)
  (condition-case nil
      (save-excursion
...
(defun cperl6-forward-to-end-of-expr (&optional lim)
  (let ((p (point))))
  (condition-case nil
      (progn
	(while (and (< (point) (or lim (point-max)))
		    (not (cperl6-at-end-of-expr)))
	  (forward-sexp 1)))
    (error nil)))

(defun cperl6-backward-to-start-of-continued-exp (lim)
  (if (memq (preceding-char) (append ")]}\"'`" nil))
      (forward-sexp -1))
  (beginning-of-line)
  (if (<= (point) lim)
...
(defun cperl6-after-block-and-statement-beg (lim)
...
If `cperl6-indent-region-fix-constructs', will improve spacing on
conditional/loop constructs."
  ...
  (save-excursion
	...
	(setq tmp-end (point-marker)))
      (if cperl6-indent-region-fix-constructs
	  (cperl6-fix-line-spacing tmp-end))
      (cperl6-indent-region (point) tmp-end))))

(defun cperl6-fix-line-spacing (&optional end parse-data)
...
Returns some position at the last line."
  ...
  (let ((ee (save-excursion (end-of-line) (point)))
	(cperl6-indent-region-fix-constructs
	 (or cperl6-indent-region-fix-constructs 1))
    ...
    (save-excursion
      ...
      ;;  blah; }
      (if (not
	   (or (looking-at "[ \t]*\\(els\\(e\\|if\\)\\|continue\\|if\\|while\\|for\\(each\\)?\\|until\\|loop\\)") ; perl6-last
	       (setq have-brace (save-excursion (search-forward "}" ee t)))))
	  nil				; Do not need to do anything
	;; Looking at:
	;; }
	;; else
	(if (and cperl6-merge-trailing-else
		 (looking-at
		  "[ \t]*}[ \t]*\n[ \t\n]*\\(els\\(e\\|if\\)\\|continue\\)\\>"))
	    (progn
	      (search-forward "}")
	      ...
	      (insert (make-string cperl6-indent-region-fix-constructs ?\ ))
	      (beginning-of-line)))
	;; Looking at:
	;; }     else
	...
	;; else   {
	(if (looking-at
	     "[ \t]*}?[ \t]*\\<\\(\\els\\(e\\|if\\)\\|continue\\|unless\\|if\\|while\\|for\\(each\\)?\\|until\\|loop\\)\\>\\(\t*\\|[ \t][ \t]+\\)[^ \t\n#]") ; perl6-last
	    (progn
	      (forward-word 1)
	      (delete-horizontal-space)
	      (insert (make-string cperl6-indent-region-fix-constructs ?\ ))
	...
	(if (looking-at
	    ...
	    (progn
	      ...
	      (delete-horizontal-space)
	      (insert
	       (make-string cperl6-indent-region-fix-constructs ?\ ))
	      (beginning-of-line)))
	;; Looking at:
	;; } foreach my $var ()    {
	(if (looking-at
	     "[ \t]*\\(}[ \t]*\\)?\\<\\(\\els\\(e\\|if\\)\\|continue\\|if\\|unless\\|while\\|loop\\|for\\(each\\)?\\(\\([ \t]+\\(my\\|local\\|our\\|state\\)\\)?[ \t]*\\$[_a-zA-Z0-9]+\\)?\\|until\\)\\>\\([ \t]*(\\|[ \t\n]*{\\)\\|[ \t]*{") ; perl6-last
	    (progn
		  ...
		  (progn
		    (cond
		     ...
		     ((looking-at "\\(\t*\\| [ \t]+\\){")
		      (delete-horizontal-space)
		      (if (if ml
			      cperl6-extra-newline-before-brace-multiline
			    cperl6-extra-newline-before-brace)
			  (progn
			    (delete-horizontal-space)
			    (insert "\n")
			    (setq ret (point))
			    (if (cperl6-indent-line parse-data)
				(progn
				  (cperl6-fix-line-spacing end parse-data)
				  (setq ret (point)))))
			(insert
			 (make-string cperl6-indent-region-fix-constructs ?\ ))))
		     ((and (looking-at "[ \t]*\n")
			...
			(progn
			  ...
			  (if (or cperl6-break-one-line-blocks-when-indent ml)
				...
				(setq ret (point))
				(if (cperl6-indent-line parse-data)
				    (setq ret (cperl6-fix-line-spacing end parse-data)))))))))))
	(beginning-of-line)
	...
	;; } blah
	(if (and
	     cperl6-fix-hanging-brace-when-indent
	     have-brace
		 ...
		 (progn
		   ...
		   (if (and (<= (point) pp)
			    (eq (preceding-char) ?\} )
			    (cperl6-after-block-and-statement-beg (point-min)))
		       t
		     (goto-char p)
		     nil))
	       (error nil)))
...
(defvar cperl6-update-start)		; Do not need to make them local
(defvar cperl6-update-end)
(defun cperl6-delay-update-hook (beg end old-len)
  (setq cperl6-update-start (min beg (or cperl6-update-start (point-max))))
  (setq cperl6-update-end (max end (or cperl6-update-end (point-min)))))

(defun cperl6-indent-region (start end)
  "Simple variant of indentation of region in Cperl6 mode.
...
conditional/loop constructs."
  (interactive "r")
  (cperl6-update-syntaxification end end)
  (save-excursion
    (let (cperl6-update-start cperl6-update-end (h-a-c after-change-functions))
      (let ((indent-info (if cperl6-emacs-can-parse
	...
	(if h-a-c (add-hook 'after-change-functions 'cperl6-delay-update-hook))
	(goto-char start)
	(setq old-comm-indent (and (cperl6-to-comment-or-eol)
				   (current-column))
	...
	(while (and (<= (point) end) (not (eobp))) ; bol to check start
	  ...
	  (if (or
		  ...
		  (let ((comment-column new-comm-indent))
		    (indent-for-comment)))
	    (progn
	      (setq i (cperl6-indent-line indent-info))
	      (or comm
		  (not i)
		  (progn
		    (if cperl6-indent-region-fix-constructs
			(goto-char (cperl6-fix-line-spacing end indent-info)))
		    (if (setq old-comm-indent
			      (and (cperl6-to-comment-or-eol)
      ...
      (and after-change-functions
	   cperl6-update-end
	   (save-excursion
	     (goto-char cperl6-update-end)
	     (insert " ")
	     (delete-char -1)
	     (goto-char cperl6-update-start)
	     (insert " ")
	     (delete-char -1))))))

;; Stolen from lisp-mode with a lot of improvements

(defun cperl6-fill-paragraph (&optional justify iteration)
  "Like \\[fill-paragraph], but handle Cperl6 comments.
	...
	(let ((paragraph-start (concat paragraph-start "\\|^[ \t#]*$"))
	  ...
	  (progn
	    ...
	    (if (or (= (current-column) c) iteration) nil
	      ...
	      (cperl6-fill-paragraph justify t))))))
  t)

(defun cperl6-do-auto-fill ()
  ...
  (if (> (save-excursion
	 ...
	 fill-column)
      (let ((c (save-excursion (beginning-of-line)
			       (cperl6-to-comment-or-eol) (point)))
	    (s (memq (following-char) '(?\ ?\t))) marker)
	(if (>= c (point)) nil
	  (setq marker (point-marker))
	  (cperl6-fill-paragraph)
	  (goto-char marker)
...
(defun cperl6-imenu-addback (lst &optional isback name)
  ...
  (cond ((not cperl6-imenu-addback) lst)
	(t
	 (or name
	     (setq name "+++BACK+++"))
	 (mapcar (lambda (elt)
		   (if (and (listp elt) (listp (cdr elt)))
			 ...
			 (setcdr elt (cons (cons name lst)
					   (cdr elt)))
			 (cperl6-imenu-addback (cdr elt) t name))))
		 (if isback (cdr lst) lst))
	 lst)))

(defun cperl6-imenu--create-perl-index (&optional regexp)
  (require 'cl)
  ...
  (let ((index-alist '()) (index-pack-alist '()) (index-pod-alist '())
    ...
    (progn ;;save-match-data
      (while (re-search-forward
	      (or regexp cperl6-imenu--function-name-regexp-perl)
	      nil t)
	(or noninteractive
    ...
    (and (or index-pack-alist index-pod-alist
	 ...
	 (push (cons "+Unsorted List+..."
	       ...
	       index-alist))
    (cperl6-imenu-addback index-alist)))


;; Suggested by Mark A. Hershberger
(defun cperl6-outline-level ()
...

(defvar cperl6-compilation-error-regexp-alist
...
(if (fboundp 'eval-after-load)
	...
	"mode-compile"
      '(setq perl-compilation-error-regexp-alist
	     cperl6-compilation-error-regexp-alist)))


(defun cperl6-windowed-init ()
  "Initialization under windowed version."
  (if (or (featurep 'ps-print) cperl6-faces-init)
      ;; Need to init anyway:
      (or cperl6-faces-init (cperl6-init-faces))
    (add-hook 'font-lock-mode-hook
	      (function
	       (lambda ()
		 (if (memq major-mode '(perl-mode cperl6-mode))
		     (progn
		       (or cperl6-faces-init (cperl6-init-faces)))))))
    (if (fboundp 'eval-after-load)
	(eval-after-load
...
(defun cperl6-load-font-lock-keywords ()
  (or cperl6-faces-init (cperl6-init-faces))
  perl-font-lock-keywords)

(defun cperl6-load-font-lock-keywords-1 ()
  (or cperl6-faces-init (cperl6-init-faces))
  perl-font-lock-keywords-1)

(defun cperl6-load-font-lock-keywords-2 ()
  (or cperl6-faces-init (cperl6-init-faces))
  ...
  (condition-case errs
	...
	(let (t-font-lock-keywords t-font-lock-keywords-1 font-lock-anchored)
	   ...
	   (list
	    ...
	    (list
	     (concat
	      ...
	      ;; "not" "none" "oct" "one" "open" "opendir" "or" "ord" "pack" "perl" "pipe"
	      ...
	      "a\\(bs\\|ccept\\|tan2\\|l\\(arm\\|l\\)\\|n\\(d\\|y\\)\\|s\\)\\|"
	      "b\\(in\\(d\\|mode\\)\\|less\\)\\|"
	      ...
	      "ctl\\|get\\)\\|kdir\\)\\|n\\(e\\|o\\(ne\\|t\\)\\)\\|o\\(pen\\(\\|dir\\)\\|ne\\|"
	    ...
	    ;; it is already done by `cperl6-find-pods-heres'
	    (list (concat "\\<\\(\\(multi\\|proto\\)[ \t]*\\)?\\(coro\\|sub\\|method\\|submethod\\)?" ; perl6: multi|proto sub methods
			  cperl6-white-and-comment-rex ; whitespace/comments
			  ...
			  "\\("
                          ...
                          cperl6-maybe-white-and-comment-rex ; whitespace/comments?
                          "[{;]")
		  5 (if cperl6-font-lock-multiline
			'(if (eq (char-after (cperl6-1- (match-end 0))) ?\{ )
			     'font-lock-function-name-face
			   'font-lock-variable-name-face)
	    ...
	    (cond ((featurep 'font-lock-extra)
		  ...
		  (font-lock-anchored
		   ...
		   (` ((, (concat "\\<\\(has\\|my\\|local\\|our\\|state\\)"
		       ...
		       (5 (, (if cperl6-font-lock-multiline
				 'font-lock-variable-name-face
			       '(progn  (setq cperl6-font-lock-multiline-start
					      (match-beginning 0))
					'font-lock-variable-name-face))))
		       ((, (concat "\\="
				   cperl6-maybe-white-and-comment-rex
			...
			(, (if cperl6-font-lock-multiline
				...
				;; so cperl6-font-lock-multiline-start is nil
				(if (or (not cperl6-font-lock-multiline-start)
					(> 2 (count-lines
					      cperl6-font-lock-multiline-start
					      (point))))
				    nil
				  (put-text-property
				   (1+ cperl6-font-lock-multiline-start) (point)
				   'syntax-type 'multiline))
				(setq cperl6-font-lock-multiline-start nil))))
			(3 font-lock-variable-name-face)))))
		  (t '("^[ \t{}]*\\(has\\|my\\|local\\|our\\|state\\)[ \t]*\\(([ \t]*\\)?\\([$@%*][a-zA-Z0-9_:]+\\)"
		       3 font-lock-variable-name-face)))
	    '("\\<for\\(each\\)?\\([ \t]+\\(has\\|my\\|local\\|our\\|state\\)\\)?[ \t]*\\(\\$[a-zA-Z_][a-zA-Z_0-9]*\\)[ \t]*("
	      4 font-lock-variable-name-face)))
      (if (not cperl6-use-v6)
          (setq
		  ...
		  ("\\(%\\)\\(\\$+\\([a-zA-Z_:][a-zA-Z0-9_:]*\\|[^ \t\n]\\)\\)"
		   ...
		   (2 font-lock-variable-name-face))
		       ...
		       ;;; Too much noise from \s* @s[ and friends
                  ...
                  ;;(4
                  ;; (if (cperl6-slash-is-regexp)
                  ;;    font-lock-function-name-face 'default) nil t))
                  )))
        (setq                            ; perl6: no whitespace allowed before subscripts in Perl6
         t-font-lock-keywords-1
         (and (fboundp 'turn-on-font-lock) ; Check for newer font-lock
              ;; not yet as of XEmacs 19.12, works with 21.1.11
              (or
               ...
               (and (string< "21.1.10" emacs-version)
                    (string< emacs-version "21.1.2")))
              '(
                ("\\(\\([@%]\\|\$#\\)[a-zA-Z_:.^*+?=!][a-zA-Z0-9_:]*\\)" 1 ; perl6: Twigils %.name %:name %^name, %*name, %+name, %?name, %=name, %!name
                 (if (eq (char-after (match-beginning 2)) ?%)
                     cperl6-hash-face
                   cperl6-array-face)
                 t)                   ; arrays and hashes
	  ...
	  (setq perl-font-lock-keywords-1
		...
		perl-font-lock-keywords-2 (append
					   perl-font-lock-keywords-1
					   t-font-lock-keywords-1)))
	(if (fboundp 'ps-print-buffer) (cperl6-ps-print-init))
	(if (or (featurep 'choose-color) (featurep 'font-lock-extra))
		...
		(list 'font-lock-constant-face
		      ...
		      nil
		      [nil		nil		t		t	t])
		(list 'cperl6-nonoverridable-face
	  ...
	  (defvar cperl6-guessed-background nil
	    "Display characteristics as guessed by cperl6.")
	  ;;	  (or (fboundp 'x-color-defined-p)
	  ...
	  (cperl6-force-face font-lock-constant-face
			    "Face for constant and label names")
	  (cperl6-force-face font-lock-variable-name-face
			    "Face for variable names")
	  (cperl6-force-face font-lock-type-face
			    "Face for data types")
	  (cperl6-force-face cperl6-nonoverridable-face
			    "Face for data types from another group")
	  (cperl6-force-face font-lock-warning-face
			    "Face for things which should stand out")
	  (cperl6-force-face font-lock-comment-face
			    "Face for comments")
	  (cperl6-force-face font-lock-function-name-face
			    "Face for function names")
	  (cperl6-force-face cperl6-hash-face
			    "Face for hashes")
	  (cperl6-force-face cperl6-array-face
			    "Face for arrays")
	  ...
	  ;;(or (boundp 'cperl6-nonoverridable-face)
	  ;;    (defconst cperl6-nonoverridable-face
	  ;;	'cperl6-nonoverridable-face
	  ;;	"Face to use for data types from another group."))
	  ;;(if (not cperl6-xemacs-p) nil
	  ...
	  (if (and
	       (not (cperl6-is-face 'cperl6-array-face))
	       (cperl6-is-face 'font-lock-emphasized-face))
	      (copy-face 'font-lock-emphasized-face 'cperl6-array-face))
	  (if (and
	       (not (cperl6-is-face 'cperl6-hash-face))
	       (cperl6-is-face 'font-lock-other-emphasized-face))
	      (copy-face 'font-lock-other-emphasized-face
			 'cperl6-hash-face))
	  (if (and
	       (not (cperl6-is-face 'cperl6-nonoverridable-face))
	       (cperl6-is-face 'font-lock-other-type-face))
	      (copy-face 'font-lock-other-type-face
			 'cperl6-nonoverridable-face))
	  ;;(or (boundp 'cperl6-hash-face)
	  ;;    (defconst cperl6-hash-face
	  ;;	'cperl6-hash-face
	  ;;	"Face to use for hashes."))
	  ;;(or (boundp 'cperl6-array-face)
	  ;;    (defconst cperl6-array-face
	  ;;	'cperl6-array-face
...
;;;;			 (function (lambda (face) (boundp face))))))
	    (defvar cperl6-guessed-background
	      (if (and (boundp 'font-lock-display-type)
		       (eq font-lock-display-type 'grayscale))
		  'gray
		background)
	      "Background as guessed by Cperl6 mode")
	    (and (not (cperl6-is-face 'font-lock-constant-face))
		 (cperl6-is-face 'font-lock-reference-face)
		 (copy-face 'font-lock-reference-face 'font-lock-constant-face))
	    (if (cperl6-is-face 'font-lock-type-face) nil
	    ...
	    (if (cperl6-is-face 'cperl6-nonoverridable-face)
		nil
	      (copy-face 'font-lock-type-face 'cperl6-nonoverridable-face)
	      (cond
	       ((eq background 'light)
		(set-face-foreground 'cperl6-nonoverridable-face
		...
		(set-face-foreground 'cperl6-nonoverridable-face
					 ...
					 "orchid1"
				       "orange")))))
;;;	    (if (cperl6-is-face 'font-lock-other-emphasized-face) nil
...
;;;	    (if (cperl6-is-face 'font-lock-emphasized-face) nil
...
;;;	       (t (set-face-background 'font-lock-emphasized-face "gray90"))))
	    (if (cperl6-is-face 'font-lock-variable-name-face) nil
	      (copy-face 'italic 'font-lock-variable-name-face))
	    (if (cperl6-is-face 'font-lock-constant-face) nil
	      (copy-face 'italic 'font-lock-constant-face))))
	(setq cperl6-faces-init t))
    (error (message "cperl6-init-faces (ignored): %s" errs))))


(defun cperl6-ps-print-init ()
  "Initialization of `ps-print' components for faces used in Cperl6."
  (eval-after-load "ps-print"
    '(setq ps-bold-faces
	   ;; 			font-lock-variable-name-face
	   ;;			font-lock-constant-face
	   (append '(cperl6-array-face
		     cperl6-hash-face)
		   ps-bold-faces)
	   ps-italic-faces
	   ;;			font-lock-constant-face
	   (append '(cperl6-nonoverridable-face
		     cperl6-hash-face)
		   ps-italic-faces)
	   ps-underlined-faces
	   ;;	     font-lock-type-face
...
(defun cperl6-ps-print (&optional file)
  "Pretty-print in Cperl6 style.
...
Style of printout regulated by the variable `cperl6-ps-print-face-properties'."
  (interactive)
  (or file
  ...
  (let ((ps-print-color-p t)
    ...
    (cperl6-ps-extend-face-list cperl6-ps-print-face-properties)
...
;;; 		  font-lock-other-emphasized-face
;;; 		  cperl6-hash-face
;;; 		  cperl6-nonoverridable-face font-lock-type-face)
;;; 		ps-underlined-faces))
;;;   (cons 'font-lock-type-face ps-underlined-faces))


(if (cperl6-enable-font-lock) (cperl6-windowed-init))

(defconst cperl6-styles-entries
  '(cperl6-indent-level cperl6-brace-offset cperl6-continued-brace-offset
    cperl6-label-offset cperl6-extra-newline-before-brace
    cperl6-merge-trailing-else
    cperl6-continued-statement-offset))

(defconst cperl6-style-alist
  '(("Cperl6"			     ; =GNU without extra-newline-before-brace
     (cperl6-indent-level               .  2)
     (cperl6-brace-offset               .  0)
     (cperl6-continued-brace-offset     .  0)
     (cperl6-label-offset               . -2)
     (cperl6-extra-newline-before-brace .  nil)
     (cperl6-merge-trailing-else	       .  t)
     (cperl6-continued-statement-offset .  2))
    ("PerlStyle"			; Cperl6 with 4 as indent
    ...
    ("GNU"
     ...
     (cperl6-continued-brace-offset     .  0)
     (cperl6-label-offset               . -2)
     (cperl6-extra-newline-before-brace .  t)
     (cperl6-merge-trailing-else	       .  nil)
     (cperl6-continued-statement-offset .  2))
    ("K&R"
     (cperl6-indent-level               .  5)
     (cperl6-brace-offset               .  0)
     (cperl6-continued-brace-offset     . -5)
     (cperl6-label-offset               . -5)
     ;;(cperl6-extra-newline-before-brace .  nil) ; ???
     (cperl6-merge-trailing-else	       .  nil)
     (cperl6-continued-statement-offset .  5))
    ("BSD"
     (cperl6-indent-level               .  4)
     (cperl6-brace-offset               .  0)
     (cperl6-continued-brace-offset     . -4)
...
Should be used via `cperl6-set-style' or via Perl menu.")

(defun cperl6-set-style (style)
  "Set Cperl6 mode variables to use one of several different indentation styles.
The arguments are a string representing the desired style.
The list of styles is in `cperl6-style-alist', available styles
are GNU, K&R, BSD, C++ and Whitesmith.

The current value of style is memorized (unless there is a memorized
data already), may be restored by `cperl6-set-style-back'.
...
side-effect of memorizing only."
  ...
  (or cperl6-old-style
      (setq cperl6-old-style
	    (mapcar (function
		     (lambda (name)
		       (cons name (eval name))))
		    cperl6-styles-entries)))
  (let ((style (cdr (assoc style cperl6-style-alist))) setting str sym)
    (while style
      (setq setting (car style) style (cdr style))
      (set (car setting) (cdr setting)))))

(defun cperl6-set-style-back ()
  "Restore a style memorised by `cperl6-set-style'."
  (interactive)
  (or cperl6-old-style (error "The style was not changed"))
  (let (setting)
    (while cperl6-old-style
      (setq setting (car cperl6-old-style)
	    cperl6-old-style (cdr cperl6-old-style))
      (set (car setting) (cdr setting)))))

(defun cperl6-check-syntax ()
  (interactive)
  (require 'mode-compile)
  (let ((perl-dbg-flags (concat cperl6-extra-perl-args " -wc")))
    (eval '(mode-compile))))		; Avoid a warning

(defun cperl6-info-buffer (type)
  ...
  (let* ((bname (if type "*info-perl-var*" "*info-perl*"))
	...
	(save-window-excursion
	  (info))
	(Info-find-node cperl6-info-page (if type "perlvar" "perlfunc"))
	(set-buffer "*info*")
	(rename-buffer bname)
	(cond (oldbuf
...
(defun cperl6-word-at-point (&optional p)
  "Return the word at point or at P."
  (save-excursion
    (if p (goto-char p))
    (or (cperl6-word-at-point-hard)
	(progn
	  (require 'etags)
...
(defun cperl6-info-on-command (command)
  "Show documentation for Perl command COMMAND in other window.
If perl-info buffer is shown in some frame, uses this frame.
Customized by setting variables `cperl6-shrink-wrap-info-frame',
`cperl6-max-help-size'."
  (interactive
   (let* ((default (cperl6-word-at-point))
	  (read (read-string
  ...
  (let ((buffer (current-buffer))
	...
	(progn
	  ...
	  (cond ((if not-loner cperl6-max-help-size
		   cperl6-shrink-wrap-info-frame)
		 (setq height
		       (+ 2
			  (count-lines
		       ...
		       max-height
		       (if not-loner
			   (/ (* (- frheight 3) cperl6-max-help-size) 100)
...
(defun cperl6-info-on-current-command ()
  "Show documentation for Perl command at point in other window."
  (interactive)
  (cperl6-info-on-command (cperl6-word-at-point)))

(defun cperl6-imenu-info-imenu-search ()
  (if (looking-at "^-X[ \t\n]") nil
    (re-search-backward
     "^\n\\([-a-zA-Z_]+\\)[ \t\n]")
    (forward-line 1)))

(defun cperl6-imenu-info-imenu-name ()
  (buffer-substring
   (match-beginning 1) (match-end 1)))

(defun cperl6-imenu-on-info ()
...
Opens Perl Info buffer if needed."
  ...
  (let* ((buffer (current-buffer))
	 ...
	 (index-item (save-restriction
		       (save-window-excursion
			 (set-buffer (cperl6-info-buffer nil))
			 (setq imenu-create-index-function
			       'imenu-default-create-index-function
			       imenu-prev-index-position-function
			       'cperl6-imenu-info-imenu-search
			       imenu-extract-index-name-function
			       'cperl6-imenu-info-imenu-name)
...
(defun cperl6-lineup (beg end &optional step minshift)
...
If STEP is nil, `cperl6-lineup-step' will be used
\(or `cperl6-indent-level', if `cperl6-lineup-step' is `nil').
Will not move the position at the start to the left."
  ...
  (let (search col tcol seen b)
	...
	(setq search (regexp-quote (char-to-string (following-char)))))
      (setq step (or step cperl6-lineup-step cperl6-indent-level))
	...
	(if (> tcol col) (setq col tcol)))
      ...
      (while
	  (progn
	    (cperl6-make-indent col)
	    (beginning-of-line 2)
	    (and (< (point) end)
		 (re-search-forward search end t)
		 (goto-char (match-beginning 0)))))))) ; No body

(defun cperl6-etags (&optional add all files) ;; NOT USED???
...
(defun cperl6-toggle-auto-newline ()
  "Toggle the state of `cperl6-auto-newline'."
  (interactive)
  (setq cperl6-auto-newline (not cperl6-auto-newline))
  (message "Newlines will %sbe auto-inserted now."
	   (if cperl6-auto-newline "" "not ")))

(defun cperl6-toggle-abbrev ()
  "Toggle the state of automatic keyword expansion in Cperl6 mode."
  (interactive)
...
(defun cperl6-toggle-electric ()
  "Toggle the state of parentheses doubling in Cperl6 mode."
  (interactive)
  (setq cperl6-electric-parens (if (cperl6-val 'cperl6-electric-parens) 'null t))
  (message "Parentheses will %sbe auto-doubled now."
	   (if (cperl6-val 'cperl6-electric-parens) "" "not ")))

(defun cperl6-toggle-autohelp ()
  "Toggle the state of Auto-Help on Perl constructs (put in the message area).
Delay of auto-help controlled by `cperl6-lazy-help-time'."
  (interactive)
  (if (fboundp 'run-with-idle-timer)
      (progn
	(if cperl6-lazy-installed
	    (cperl6-lazy-unstall)
	  (cperl6-lazy-install))
	(message "Perl help messages will %sbe automatically shown now."
		 (if cperl6-lazy-installed "" "not ")))
    (message "Cannot automatically show Perl help messages - run-with-idle-timer missing.")))

(defun cperl6-toggle-construct-fix ()
  "Toggle whether `indent-region'/`indent-sexp' fix whitespace too."
  (interactive)
  (setq cperl6-indent-region-fix-constructs
	(if cperl6-indent-region-fix-constructs
	    nil
	  1))
  (message "indent-region/indent-sexp will %sbe automatically fix whitespace."
	   (if cperl6-indent-region-fix-constructs "" "not ")))

(defun cperl6-toggle-set-debug-unwind (arg &optional backtrace)
...
by Cperl6."
  (interactive "P")
  (or arg
      (setq arg (if (eq cperl6-syntaxify-by-font-lock
			(if backtrace 'backtrace 'message)) 0 1)))
  (setq arg (if (> arg 0) (if backtrace 'backtrace 'message) t))
  (setq cperl6-syntaxify-by-font-lock arg)
  (message "Debugging messages of syntax unwind %sabled."
	   (if (eq arg t) "dis" "en")))

;;;; Tags file creation.

(defvar cperl6-tmp-buffer " *cperl6-tmp*")

(defun cperl6-setup-tmp-buf ()
  (set-buffer (get-buffer-create cperl6-tmp-buffer))
  (set-syntax-table cperl6-mode-syntax-table)
  (buffer-disable-undo)
  (auto-fill-mode 0)
  (if cperl6-use-syntax-table-text-property-for-tags
      (progn
	(make-local-variable 'parse-sexp-lookup-properties)
	;; Do not introduce variable if not needed, we check it!
	(set 'parse-sexp-lookup-properties t))))

(defun cperl6-xsub-scan ()
  (require 'cl)
  (require 'imenu)
  (let ((index-alist '())
...
(defvar cperl6-unreadable-ok nil)

(defun cperl6-find-tags (ifile xs topdir)
  (let ((b (get-buffer cperl6-tmp-buffer)) ind lst elt pos ret rel
	(cperl6-pod-here-fontify nil) f file)
    (save-excursion
      (if b (set-buffer b)
	(cperl6-setup-tmp-buf))
      (erase-buffer)
      (condition-case err
	  (setq file (car (insert-file-contents ifile)))
	(error (if cperl6-unreadable-ok nil
		 (if (y-or-n-p
		      (format "File %s unreadable.  Continue? " ifile))
		     (setq cperl6-unreadable-ok t)
		   (error "Aborting: unreadable file %s" ifile)))))
      (if (not file)
	  (message "Unreadable file %s" ifile)
	(message "Scanning file %s ..." file)
	(if (and cperl6-use-syntax-table-text-property-for-tags
		 (not xs))
	    (condition-case err		; after __END__ may have garbage
		(cperl6-find-pods-heres nil nil noninteractive)
	      (error (message "While scanning for syntax: %s" err))))
	(if xs
	    (setq lst (cperl6-xsub-scan))
	  (setq ind (cperl6-imenu--create-perl-index))
	...
	(while lst
	  ...
	  (if elt
		...
		(if (and (string-match "^[_a-zA-Z]+::" (car elt))
			 (string-match "^sub[ \t]+\\([_a-zA-Z]+\\)[^:_a-zA-Z]" ; perl6: TODO: multi|proto submethods
				       (elt elt 3)))
...
(defun cperl6-add-tags-recurse-noxs ()
  "Add to TAGS data for \"pure\" Perl files in the current directory and kids.
Use as
  emacs -batch -q -no-site-file -l emacs/cperl6-mode.el \
        -f cperl6-add-tags-recurse-noxs
"
  (cperl6-write-tags nil nil t t nil t))

(defun cperl6-add-tags-recurse-noxs-fullpath ()
...
is relocated, the file TAGS inside it breaks). Use as
  emacs -batch -q -no-site-file -l emacs/cperl6-mode.el \
        -f cperl6-add-tags-recurse-noxs-fullpath
"
  (cperl6-write-tags nil nil t t nil t ""))

(defun cperl6-add-tags-recurse ()
  "Add to TAGS file data for Perl files in the current directory and kids.
Use as
  emacs -batch -q -no-site-file -l emacs/cperl6-mode.el \
        -f cperl6-add-tags-recurse
"
  (cperl6-write-tags nil nil t t))

(defun cperl6-write-tags (&optional file erase recurse dir inbuffer noxs topdir)
  ...
  (let ((tags-file-name "TAGS")
	...
	(let ((files
		   ...
		   (directory-files file t
				    (if recurse nil cperl6-scan-files-regexp)
				    t)
		 (error
		  (if cperl6-unreadable-ok nil
		    (if (y-or-n-p
	  ...
	  (mapcar (function
		   (lambda (file)
		     (cond
		      ((string-match cperl6-noscan-files-regexp file)
		       nil)
		      ((not (file-directory-p file))
		       (if (string-match cperl6-scan-files-regexp file)
			   (cperl6-write-tags file erase recurse nil t noxs topdir)))
		      ((not recurse) nil)
		      (t (cperl6-write-tags file erase recurse t t noxs topdir)))))
		  files)))
       (t
			   ...
			   (delete-region (point)
					  (save-excursion
						...
						(- (point) 2)
					      (point-max)))))
		       (goto-char (point-max)))))
	      (insert (cperl6-find-tags file xs topdir))))))
...
(defvar cperl6-tags-hier-regexp-list
  (concat
   "^\\("
...
(defvar cperl6-hierarchy '(() ())
  "Global hierarchy of classes.")

(defun cperl6-tags-hier-fill ()
  ;; Suppose we are in a tag table cooked by cperl6.
  (goto-char 1)
  (let (type pack name pos line chunk ord cons1 file str info fileind)
    (while (re-search-forward cperl6-tags-hier-regexp-list nil t)
	  ...
	  (progn
	    (setq ;;str (buffer-substring (match-beginning 1) (match-end 1))
		  ...
		  info (cperl6-etags-snarf-tag file line))
	    ;; Move back
	    (forward-char -1)
	    ;; Make new member of hierarchy name ==> file ==> pos if needed
	    (if (setq cons1 (assoc name (nth ord cperl6-hierarchy)))
		;; Name known
		(setcdr cons1 (cons (cons fileind (vector file info))
	      ...
	      (if pack
		  (setcar (cdr cperl6-hierarchy)
			  (cons cons1 (nth 1 cperl6-hierarchy)))
		(setcar cperl6-hierarchy
			(cons cons1 (car cperl6-hierarchy)))))))
      (end-of-line))))

(defun cperl6-tags-hier-init (&optional update)
...
One may build such TAGS files from Cperl6 mode menu."
  (interactive)
  (require 'etags)
  (require 'imenu)
  (if (or update (null (nth 2 cperl6-hierarchy)))
      (let ((remover (function (lambda (elt) ; (name (file1...) (file2..))
				 (or (nthcdr 2 elt)
				     ;; Only in one file
				     (setcdr elt (cdr (nth 1 elt)))))))
	    pack name cons1 to l1 l2 l3 l4 b)
	;; (setq cperl6-hierarchy '(() () ())) ; Would write into '() later!
	(setq cperl6-hierarchy (list l1 l2 l3))
	(if cperl6-xemacs-p		; Not checked
	    (progn
	      (or tags-file-name
	...
	(mapcar remover (car cperl6-hierarchy))
	(mapcar remover (nth 1 cperl6-hierarchy))
	(setq to (list nil (cons "Packages: " (nth 1 cperl6-hierarchy))
		       (cons "Methods: " (car cperl6-hierarchy))))
	(cperl6-tags-treeify to 1)
	(setcar (nthcdr 2 cperl6-hierarchy)
		(cperl6-menu-to-keymap (cons '("+++UPDATE+++" . -999) (cdr to))))
	(message "Updating list of classes: done, requesting display...")
	;;(cperl6-imenu-addback (nth 2 cperl6-hierarchy))
	))
  (or (nth 2 cperl6-hierarchy)
      (error "No items found"))
  (setq update
;;;	(imenu-choose-buffer-index "Packages: " (nth 2 cperl6-hierarchy))
	(if (if (fboundp 'display-popup-menus-p)
		(let ((f 'display-popup-menus-p))
		  (funcall f))
	      window-system)
	    (x-popup-menu t (nth 2 cperl6-hierarchy))
	  (require 'tmm)
	  (tmm-prompt (nth 2 cperl6-hierarchy))))
  ...
  (if (vectorp update)
	...
	(cperl6-etags-goto-tag-location (elt update 1))))
  (if (eq update -999) (cperl6-tags-hier-init t)))

(defun cperl6-tags-treeify (to level)
  ...
  (let* ((regexp (concat "^\\(" (mapconcat
	...
	(mapcar (function (lambda (elt)
			  (cperl6-tags-treeify elt (1+ level))))
		(cdr to)))
...
(defun cperl6-list-fold (list name limit)
...
(defun cperl6-menu-to-keymap (menu &optional name)
  (let (list)
    (cons 'keymap
	  (mapcar
	   (function
	    (lambda (elt)
	      (cond ((listp (cdr elt))
		     (setq list (cperl6-list-fold
				 (cdr elt) (car elt) imenu-max-items))
		     (cons nil
			   (cons (car elt)
				 (cperl6-menu-to-keymap list))))
		    (t
		     (list (cdr elt) (car elt) t))))) ; t is needed in 19.34
	   (cperl6-list-fold menu "Root" imenu-max-items)))))


(defvar cperl6-bad-style-regexp
  (mapconcat 'identity
	     '("[^-\n\t <>=+!.&|(*/'`\"#^][-=+<>!|&^]" ; char sign
	       "[-<>=+^&|]+[^- \t\n=+<>~]") ; sign+ char
	     "\\|")
...
(defvar cperl6-not-bad-style-regexp
  (mapconcat
   'identity
   '("[^-\t <>=+]\\(--\\|\\+\\+\\)"	; var-- var++
...
(defun cperl6-next-bad-style ()
  (let (p (not-found t) (point (point)) found)
    (while (and not-found
		(re-search-forward cperl6-bad-style-regexp nil 'to-end))
      (setq p (point))
      (goto-char (match-beginning 0))
      (if (or
	   (looking-at cperl6-not-bad-style-regexp)
...

;;; Getting help
(defvar cperl6-have-help-regexp
  ;;(concat "\\("
  (mapconcat
   'identity
...
(defvar cperl6-message-on-help-error t)
(defvar cperl6-help-from-timer nil)

(defun cperl6-word-at-point-hard ()
  ...
  (cond
   ...
   ((and (eq (following-char) ?\$)
	 ...
	 (looking-at "\\$?[a-zA-Z0-9_:]+>")) ; <$fh>
    (forward-char -1)))
  (if (looking-at cperl6-have-help-regexp)
      (buffer-substring (match-beginning 0) (match-end 0))))

(defun cperl6-get-help ()
...
than a line.  Your contribution to update/shorten it is appreciated."
  ...
  (save-match-data			; May be called "inside" query-replace
    (save-excursion
      (let ((word (cperl6-word-at-point-hard)))
	(if word
	    (if (and cperl6-help-from-timer ; Bail out if not in mainland
			 ...
			 (memq (get-text-property (point) 'syntax-type)
			       '(pod here-doc format))))
		nil
	      (cperl6-describe-perl-symbol word))
	  (if cperl6-message-on-help-error
	      (message "Nothing found for %s..."
		       (buffer-substring (point) (min (+ 5 (point)) (point-max))))))))))

;;; Stolen from perl-descr.el by Johan Vromans:

(defvar cperl6-doc-buffer " *perl-doc*"
  "Where the documentation can be found.")

(defun cperl6-describe-perl-symbol (val)
...
(defvar cperl6-short-docs 'please-ignore-this-line
...
(defun cperl6-switch-to-doc-buffer ()
  "Go to the perl documentation buffer and insert the documentation."
  (interactive)
  (let ((buf (get-buffer-create cperl6-doc-buffer)))
    (if (interactive-p)
	(switch-to-buffer-other-window buf)
      (set-buffer buf))
    (if (= (buffer-size) 0)
	(progn
	  (insert (documentation-property 'cperl6-short-docs
					  'variable-documentation))
	  (setq buffer-read-only t)))))

(defun cperl6-beautify-regexp-piece (b e embed level)
  ...
  (let (s c tmp (m (make-marker)) (m1 (make-marker)) c1 spaces inline code pos)
    ...
    (setq c (if embed (current-indentation) (1- (current-column)))
	  c1 (+ c (or cperl6-regexp-indent-step cperl6-indent-level)))
    (or (looking-at "[ \t]*[\n#]")
	(progn
	  (insert "\n")))
    (goto-char e)
		...
		"\\=\\([*+?]\\|{[0-9]+\\(,[0-9]*\\)?}\\)\\??" e t))
		 ...
		 (cond
		  ((not (match-beginning 8))
		   (cperl6-beautify-regexp-piece tmp m t level))
		  ((eq (char-after (+ 2 tmp)) ?\{) ; Code
		   t)
		  ((eq (char-after (+ 2 tmp)) ?\() ; Conditional
		   (goto-char (+ 2 tmp))
		   (forward-sexp 1)
		   (cperl6-beautify-regexp-piece (point) m t level))
...
(defun cperl6-beautify-regexp (&optional deep)
  "Do it.  (Experimental, may change semantics, recheck the result.)
We suppose that the regexp is scanned already."
  (interactive "P")
  (setq deep (if deep (prefix-numeric-value deep) -1))
  (save-excursion
    (goto-char (cperl6-make-regexp-x))
    (let ((b (point)) (e (make-marker)))
      (forward-sexp 1)
      (set-marker e (1- (point)))
      (cperl6-beautify-regexp-piece b e nil deep))))

(defun cperl6-regext-to-level-start ()
  "Goto start of an enclosing group in regexp.
We suppose that the regexp is scanned already."
  (interactive)
  (let ((limit (cperl6-make-regexp-x)) done)
    (while (not done)
      (or (eq (following-char) ?\()
...
(defun cperl6-contract-level ()
  "Find an enclosing group in regexp and contract it.
\(Experimental, may change semantics, recheck the result.)
We suppose that the regexp is scanned already."
  (interactive)
  ;; (save-excursion		; Can't, breaks `cperl6-contract-levels'
  (cperl6-regext-to-level-start)
  ...
  (save-excursion
	...
	(cond
	 ((match-beginning 1)		; Skip
	  nil)
	 (t				; Group
	  (cperl6-contract-level)))))))

(defun cperl6-beautify-level (&optional deep)
...
(defun cperl6-invert-if-unless-modifiers ()
...
\(Unfinished.)"
  ...
  (let (A B pre-B post-B pre-if post-if pre-A post-A if-string
    ...
    (and (= (char-syntax (preceding-char)) ?w)
	 (forward-sexp -1))
    (setq pre-if (point))
    (cperl6-backward-to-start-of-expr)
    (setq pre-B (point))
    (forward-sexp 1)		; otherwise forward-to-end-of-expr is NOP
    (cperl6-forward-to-end-of-expr)
    (setq post-A (point))
    (goto-char pre-if)
    (or (looking-at w-rex)
	;; Find the position
    ...
    (insert "\n" B ";\n}")
    (and (looking-at "[ \t]*#") (cperl6-indent-for-comment))
	...
	(if is-block
	    (progn
	      ...
	      (if (and (eq (following-char) ?\{ )
		       (progn
			 (cperl6-backward-to-noncomment post-A)
			 (eq (preceding-char) ?\) )))
		  (if (condition-case nil
		    ...
		    (goto-char (1- post-B))
		    (cperl6-backward-to-noncomment pre-B)
		    (if (eq (preceding-char) ?\;)
			(forward-char -1))
		    ...
		    (and (equal B "")
			 (setq B "1"))
		    (goto-char (1- post-A))
		    (cperl6-backward-to-noncomment pre-A)
		    (or (looking-at "[ \t\n]*)")
			(goto-char (1- post-A)))
		    (setq p (point))
		    (goto-char (1+ pre-A))
	  ...
	  (forward-sexp -1)
	  (cperl6-invert-if-unless-modifiers)))
...
;;;###autoload
(defun cperl6-perldoc (word)
  "Run `perldoc' on WORD."
  (interactive
   (list (let* ((default-entry (cperl6-word-at-point))
                (input (read-string
...
;;;###autoload
(defun cperl6-perldoc-at-point ()
  "Run a `perldoc' on the word around point."
  (interactive)
  (cperl6-perldoc (cperl6-word-at-point)))
      ...
      (let ((process-environment (copy-sequence process-environment)))
        ...
        (set-process-sentinel
         (start-process pod2man-program buffer "sh" "-c"
                        (format (cperl6-pod2man-build-command) pod2man-args))
         'Man-bgproc-sentinel)))))

;;; Updated version by him too
(defun cperl6-build-manpage ()
...
(defun cperl6-pod2man-build-command ()
...
(defun cperl6-next-interpolated-REx-1 ()
...
(defun cperl6-next-interpolated-REx-0 ()
  "Move point to next REx which has interpolated parts without //o."
  (interactive "")
  (cperl6-next-interpolated-REx 0))

(defun cperl6-next-interpolated-REx (&optional skip beg limit)
...
(defun cperl6-here-doc-spell (&optional beg end)
  "Spell-check HERE-documents in the Perl buffer.
If a region is highlighted, restricts to the region."
  (interactive "")
  (cperl6-pod-spell t beg end))

(defun cperl6-pod-spell (&optional do-heres beg end)
...
If a region is highlighted, restricts to the region."
  ...
  (save-excursion
    (let (beg end)
      (if (cperl6-mark-active)
	  (setq beg (min (mark) (point))
		end (max (mark) (point)))
	(setq beg (point-min)
	      end (point-max)))
      (cperl6-map-pods-heres (function
			     (lambda (s e p)
			       (if do-heres
...
(defun cperl6-map-pods-heres (func &optional prop s end)
...
(defun cperl6-get-here-doc-region (&optional pos pod)
...
(defun cperl6-facemenu-add-face-function (face end)
...
Such requests are usually bound to M-o LETTER."
  ...
  (cdr (or (assq face '((bold . "B<")
			...
			(bold-italic . "B<I<")
			(underline . "C<")))
	   (error "Face %s not configured for cperl6-mode"
		  face))))

(defun cperl6-time-fontification (&optional l step lim)
...
start with default arguments, then refine the slowdown regions."
  ...
  (let* ((timems (function (lambda ()
    ...
    (message "cperl6-mode at %s: %s" l tot)
...
(defun cperl6-emulate-lazy-lock (&optional window-size)
...
goes backwards; default is -50.  This function is not Cperl6-specific; it
...

(defun cperl6-lazy-install ())		; Avoid a warning
(defun cperl6-lazy-unstall ())		; Avoid a warning

(if (fboundp 'run-with-idle-timer)
    (progn
      (defvar cperl6-help-shown nil
	"Non-nil means that the help was already shown now.")

      (defvar cperl6-lazy-installed nil
	"Non-nil means that the lazy-help handlers are installed now.")

      (defun cperl6-lazy-install ()
	"Switches on Auto-Help on Perl constructs (put in the message area).
Delay of auto-help controlled by `cperl6-lazy-help-time'."
	(interactive)
	(make-variable-buffer-local 'cperl6-help-shown)
	(if (and (cperl6-val 'cperl6-lazy-help-time)
		 (not cperl6-lazy-installed))
	    (progn
	      (add-hook 'post-command-hook 'cperl6-lazy-hook)
	      (run-with-idle-timer
	       (cperl6-val 'cperl6-lazy-help-time 1000000 5)
	       t
	       'cperl6-get-help-defer)
	      (setq cperl6-lazy-installed t))))

      (defun cperl6-lazy-unstall ()
	"Switches off Auto-Help on Perl constructs (put in the message area).
Delay of auto-help controlled by `cperl6-lazy-help-time'."
	(interactive)
	(remove-hook 'post-command-hook 'cperl6-lazy-hook)
	(cancel-function-timers 'cperl6-get-help-defer)
	(setq cperl6-lazy-installed nil))

      (defun cperl6-lazy-hook ()
	(setq cperl6-help-shown nil))

      (defun cperl6-get-help-defer ()
	(if (not (memq major-mode '(perl-mode cperl6-mode))) nil
	  (let ((cperl6-message-on-help-error nil) (cperl6-help-from-timer t))
	    (cperl6-get-help)
	    (setq cperl6-help-shown t))))
      (cperl6-lazy-install)))


;;; Plug for wrong font-lock:

(defun cperl6-font-lock-unfontify-region-function (beg end)
...
(defun cperl6-font-lock-fontify-region-function (beg end loudly)
...
do extra unwind via `cperl6-unwind-to-safe'."
  (save-excursion
    (goto-char beg)
    (while (and beg
		(progn
		  ...
		  (eq (get-text-property (setq beg (point)) 'syntax-type)
		      'multiline)))
      (if (setq beg (cperl6-beginning-of-property beg 'syntax-type))
	  (goto-char beg)))
    (setq beg (point))
    (goto-char end)
    (while (and end
...
(defvar cperl6-d-l nil)
(defun cperl6-fontify-syntaxically (end)
  ;; Some vars for debugging only
  ;; (message "Syntaxifying...")
  (let ((dbg (point)) (iend end) (idone cperl6-syntax-done-to)
	(istate (car cperl6-syntax-state))
	start from-start edebug-backtrace-buffer)
    (if (eq cperl6-syntaxify-by-font-lock 'backtrace)
	(progn
	  (require 'edebug)
	  (let ((f 'edebug-backtrace))
	    (funcall f))))	; Avoid compile-time warning
    (or cperl6-syntax-done-to
	(setq cperl6-syntax-done-to (point-min)
	      from-start t))
    (setq start (if (and cperl6-hook-after-change
			 (not from-start))
		    cperl6-syntax-done-to ; Fontify without change; ignore start
		  ;; Need to forget what is after `start'
		  (min cperl6-syntax-done-to (point))))
    (goto-char start)
    (beginning-of-line)
    (setq start (point))
    (and cperl6-syntaxify-unwind
	 (setq end (cperl6-unwind-to-safe t end)
	       start (point)))
    (and (> end start)
	 (setq cperl6-syntax-done-to start) ; In case what follows fails
	 (cperl6-find-pods-heres start end t nil t))
    (if (memq cperl6-syntaxify-by-font-lock '(backtrace message))
	(message "Syxify req=%s..%s actual=%s..%s done-to: %s=>%s statepos: %s=>%s"
		 dbg iend start end idone cperl6-syntax-done-to
		 istate (car cperl6-syntax-state))) ; For debugging
    nil))				; Do not iterate

(defun cperl6-fontify-update (end)
  (let ((pos (point-min)) prop posend)
    (setq end (point-max))
    (while (< pos end)
      (setq prop (get-text-property pos 'cperl6-postpone)
	    posend (next-single-property-change pos 'cperl6-postpone nil end))
      (and prop (put-text-property pos posend (car prop) (cdr prop)))
      (setq pos posend)))
  nil)					; Do not iterate

(defun cperl6-fontify-update-bad (end)
  ...
  (let* ((pos (point)) (prop (get-text-property pos 'cperl6-postpone)) posend)
    (if prop
	(setq pos (or (cperl6-beginning-of-property
		       (cperl6-1+ pos) 'cperl6-postpone)
		      (point-min))))
    (while (< pos end)
      (setq posend (next-single-property-change pos 'cperl6-postpone))
      (and prop (put-text-property pos posend (car prop) (cdr prop)))
      (setq pos posend)
      (setq prop (get-text-property pos 'cperl6-postpone))))
  nil)					; Do not iterate
...
(defun cperl6-after-change-function (beg end old-len)
  ...
  (if cperl6-syntax-done-to
      (setq cperl6-syntax-done-to (min cperl6-syntax-done-to beg))))

(defun cperl6-update-syntaxification (from to)
  (if (and cperl6-use-syntax-table-text-property
	   cperl6-syntaxify-by-font-lock
	   (or (null cperl6-syntax-done-to)
	       (< cperl6-syntax-done-to to)))
      (progn
	(save-excursion
	  (goto-char from)
...
(defvar cperl6-version
  (let ((v  "Revision: 5.20.2-Pugs "))
    (string-match ":\\s *\\([-0-9a-z.]+\\)" v)
    (substring v (match-beginning 1) (match-end 1)))
  "Version of IZ-supported Cperl6 package this file is based on.")

(provide 'cperl6-mode)

;;; cperl6-mode.el ends here
